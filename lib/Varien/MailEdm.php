 ...
 * EDM发邮件专用通道 
 */
class Varien_MailEdm extends Varien_Mime_Message
{
    /**#@+
     * @access protected
     */
    ...
    /**
     * @var Varien_Mail_Transport_Abstract
     * @static
     */
     ...
     * @var string
     */
    protected $_headerEncoding = Varien_Mime::ENCODING_QUOTEDPRINTABLE;
     ...
     * @param  Varien_Mail_Transport_Abstract $transport
     */
    public static function setDefaultTransport(Varien_Mail_Transport_Abstract $transport)
    {
     ...
     * @return Varien_Mail Implements fluent interface
     * @throws Varien_Mail_Exception for types not supported by Varien_Mime
     */
    public function setType($type)
    {
        $allowed = array(
            Varien_Mime::MULTIPART_ALTERNATIVE,
            Varien_Mime::MULTIPART_MIXED,
            Varien_Mime::MULTIPART_RELATED,
        );
        if (!in_array($type, $allowed)) {
            /**
             * @see Varien_Mail_Exception
             */
            #require_once 'Zend/Mail/Exception.php';
            throw new Varien_Mail_Exception('Invalid content type "' . $type . '"');
        }
     ...
     *
     * If not set, Varien_Mime will generate one.
     *
     * @param  string    $boundary
     * @return Varien_Mail Provides fluent interface
     */
     ...
     *
     * Either Varien_Mime::ENCODING_QUOTEDPRINTABLE or Varien_Mime::ENCODING_BASE64
     *
     * @return string
     */
     ...
     * Set the encoding of mail headers
     *
     * @param  string $encoding Varien_Mime::ENCODING_QUOTEDPRINTABLE or Varien_Mime::ENCODING_BASE64
     * @return Varien_Mail Provides fluent interface
     */
    public function setHeaderEncoding($encoding)
    {
        $allowed = array(
            Varien_Mime::ENCODING_BASE64,
            Varien_Mime::ENCODING_QUOTEDPRINTABLE
        );
        if (!in_array($encoding, $allowed)) {
             ...
             */
            #require_once 'Zend/Mail/Exception.php';
            throw new Varien_Mail_Exception('Invalid encoding "' . $encoding . '"');
        }
     ...
     * @return Varien_Mail Provides fluent interface
    */
    public function setBodyText($txt, $charset = null, $encoding = Varien_Mime::ENCODING_QUOTEDPRINTABLE)
    {
        if ($charset === null) {
            $charset = $this->_charset;
        }

        $mp = new Varien_Mime_Part($txt);
        $mp->encoding = $encoding;
        $mp->type = Varien_Mime::TYPE_TEXT;
        $mp->disposition = Varien_Mime::DISPOSITION_INLINE;
        $mp->charset = $charset;
     ...
     * @return Varien_Mail Provides fluent interface
     */
    public function setBodyHtml($html, $charset = null, $encoding = Varien_Mime::ENCODING_QUOTEDPRINTABLE)
    {
        if ($charset === null) {
            $charset = $this->_charset;
        }

        $mp = new Varien_Mime_Part($html);
        $mp->encoding = $encoding;
        $mp->type = Varien_Mime::TYPE_HTML;
        $mp->disposition = Varien_Mime::DISPOSITION_INLINE;
        $mp->charset = $charset;
    ...
    /**
     * Return Varien_Mime_Part representing body HTML
     *
     ...
     *
     * @param  Varien_Mime_Part $attachment
     * @return Varien_Mail Provides fluent interface
     */
    public function addAttachment(Varien_Mime_Part $attachment)
    {
    ...
    /**
     * Creates a Varien_Mime_Part attachment
     *
     ...
     * @return Varien_Mime_Part Newly created Varien_Mime_Part object (to allow
     * advanced settings)
     */
    public function createAttachment($body,
                                     $mimeType    = Varien_Mime::TYPE_OCTETSTREAM,
                                     $disposition = Varien_Mime::DISPOSITION_ATTACHMENT,
                                     $encoding    = Varien_Mime::ENCODING_BASE64,
                                     $filename    = null)
    {

        $mp = new Varien_Mime_Part($body);
        $mp->encoding = $encoding;
        $mp->type = $mimeType;
    ...
    {
        if (Varien_Mime::isPrintable($value) === false) {
            if ($this->getHeaderEncoding() === Varien_Mime::ENCODING_QUOTEDPRINTABLE) {
                $value = Varien_Mime::encodeQuotedPrintableHeader($value, $this->getCharset(), Varien_Mime::LINELENGTH, Varien_Mime::LINEEND);
            } else {
                $value = Varien_Mime::encodeBase64Header($value, $this->getCharset(), Varien_Mime::LINELENGTH, Varien_Mime::LINEEND);
            }
        }
     ...
     *
     * @param string $headerName
     * @return Varien_Mail Provides fluent inter
     */
     ...
     * @param  string    $name
     * @return Varien_Mail Provides fluent interface
     * @throws Varien_Mail_Exception if called subsequent times
     */
    public function setFrom($email, $name = null)
    {
        if (null !== $this->_from) {
             ...
             */
            #require_once 'Zend/Mail/Exception.php';
            throw new Varien_Mail_Exception('From Header set twice');
        }
     ...
     * @param string $name
     * @return Varien_Mail
     * @throws Varien_Mail_Exception if called more than one time
     */
    public function setReplyTo($email, $name = null)
    {
        if (null !== $this->_replyTo) {
             ...
             */
            #require_once 'Zend/Mail/Exception.php';
            throw new Varien_Mail_Exception('Reply-To Header set twice');
        }
    ...
    public function setFromToDefaultFrom() {
        ...
        if($from === null) {
            #require_once 'Zend/Mail/Exception.php';
            throw new Varien_Mail_Exception(
     ...
     * @param  string    $email
     * @return Varien_Mail Provides fluent interface
     * @throws Varien_Mail_Exception if set multiple times
     */
    public function setReturnPath($email)
    {
        if ($this->_returnPath === null) {
             ...
             */
            #require_once 'Zend/Mail/Exception.php';
            throw new Varien_Mail_Exception('Return-Path Header set twice');
        }
        return $this;
    }
    ...
    public function setSubject($subject)
    {
        if ($this->_subject === null) {
             ...
             */
            #require_once 'Zend/Mail/Exception.php';
            throw new Varien_Mail_Exception('Subject set twice');
        }
        return $this;
    }
     ...
     * Sets Date-header
     *
     * @param  timestamp|string|Varien_Date $date
     * @return Varien_Mail Provides fluent interface
     * @throws Varien_Mail_Exception if called subsequent times or wrong date format.
     */
        ...
        if ($this->_date === null) {
            if ($date === null) {
                ...
                if ($date === false || $date < 0) {
                     ...
                     */
                    #require_once 'Zend/Mail/Exception.php';
                    throw new Varien_Mail_Exception('String representations of Date Header must be ' .
                                                  'strtotime()-compatible');
                }
                $date = date('r', $date);
            } else if ($date instanceof Varien_Date) {
                $date = $date->get(Varien_Date::RFC_2822);
            } else {
                /**
                 * @see Varien_Mail_Exception
                 */
                #require_once 'Zend/Mail/Exception.php';
                throw new Varien_Mail_Exception(__METHOD__ . ' only accepts UNIX timestamps, Varien_Date objects, ' .
             ...
             */
            #require_once 'Zend/Mail/Exception.php';
            throw new Varien_Mail_Exception('Date Header set twice');
        }
        return $this;
    }
    ...
    public function setMessageId($id = true)
    {
        ...
        if ($this->_messageId === null) {
             ...
             */
            #require_once 'Zend/Mail/Exception.php';
            throw new Varien_Mail_Exception('Message-ID set twice');
        }
     ...
     * @return Varien_Mail           Provides fluent interface
     * @throws Varien_Mail_Exception on attempts to create standard headers
     */
    public function addHeader($name, $value, $append = false)
    {
        ...
        if (in_array(strtolower($name), $prohibit)) {
             ...
             */
            #require_once 'Zend/Mail/Exception.php';
            throw new Varien_Mail_Exception('Cannot set standard header from addHeader()');
        }
    ...
    {
        if ($transport === null) {
            if (!self::$_defaultTransport instanceof Varien_Mail_Transport_Abstract) {
				//这里启用SMTP发送邮件,先随机用一个帐号发，后面再做限量监控
				$collection = Mage::getResourceModel('edm/server_collection')
					->addFieldToFilter('status',1);
				$collection->getSelect()->order('rand()');
				$server =	$collection->getFirstItem();

				$host = $server->getData('host');
				$config = array(
					'port' => $server->getData('port'),
					'auth' => 'login',
					'username' => $server->getData('username'),
					'password' => $server->getData('password'),
				);
                $transport = new Varien_Mail_Transport_Smtp($host,$config);
                
                 Varien_Mail::setDefaultTransport($transport);
            } else {
