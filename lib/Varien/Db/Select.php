<?php
class Varien_Db_Select extends Magento_Db_Select
{
     ...
     * @return Varien_Db_Select This Magento_Db_Select object.
     */
    ...
    public function setPart($part, $value)
    {
        ...
        if (!array_key_exists($part, $this->_parts)) {
            throw new Varien_Exception("Invalid Select part '{$part}'");
        }
    ...
    protected function _tableCols($correlationName, $cols, $afterCorrelationName = null)
    {
        ...
        foreach ($cols as $k => $v) {
            if ($v instanceof Varien_Db_Select) {
                $cols[$k] = new Magento_Db_Expr(sprintf('(%s)', $v->assemble()));
            }
        }
    ...
    public function exists($select, $joinCondition, $isExists = true)
    {
        ...
        $select->reset(self::COLUMNS)
            ->columns(array(new Magento_Db_Expr('1')))
            ->where($joinCondition);
