<?php
class Varien_Db_Adapter_Pdo_Mysql extends Varien_Db_Adapter_Abstract implements Varien_Db_Adapter_Interface
{
    ...
    public function addColumn($tableName, $columnName, $definition, $schemaName = null)
    {
        ...
        if (is_array($definition)) {
            ...
            if (empty($definition['COMMENT'])) {
                throw new Varien_Db_Exception("Impossible to create a column without comment.");
            }
        ...
        $schemaName = null)
    {
        if (!$this->tableColumnExists($tableName, $oldColumnName, $schemaName)) {
            throw new Varien_Db_Exception(sprintf(
    ...
    {
        if (!$this->tableColumnExists($tableName, $columnName, $schemaName)) {
            throw new Varien_Db_Exception(sprintf('Column "%s" does not exist in table "%s".', $columnName, $tableName));
        }
        if (is_array($definition)) {
            $definition = $this->_getColumnDefinition($definition);
        }
    ...
    public function quoteInto($text, $value, $type = null, $count = null)
    {
        if (is_array($value) && empty($value)) {
            $value = new Magento_Db_Expr('NULL');
        }
    ...
    public function describeTable($tableName, $schemaName = null)
    {
        ...
        if ($ddl === false) {
            $ddl = array_map(
                ...
                $this->_describeTable($tableName, $schemaName)
            );
            /**
     ...
     * @return array
     */
    protected function _describeTable($tableName, $schemaName = null)
    {
        ...
        $result = $stmt->fetchAll(Varien_Db::FETCH_NUM);
        ...
        if (is_array($row)) { // Array of column-value pairs
            ...
            foreach ($data as $row) {
                if (array_diff($cols, array_keys($row))) {
                    throw new Varien_Db_Exception('Invalid data for insert');
                }
...
//        $cols = array_map(array($this, 'quoteIdentifier'), $cols);
        ...
        foreach ($fields as $k => $v) {
            ...
            if (!is_numeric($k)) {
                ...
                if ($v instanceof Magento_Db_Expr) {
                    $value = $v->__toString();
                } elseif (is_string($v)) {
    ...
    public function insertIgnore($table, array $bind)
    {
        ...
        foreach ($bind as $col => $val) {
            ...
            if ($val instanceof Magento_Db_Expr) {
                $vals[] = $val->__toString();
                unset($bind[$col]);
            } else {
    ...
    public function createTable(Varien_Db_Ddl_Table $table)
    {
        ...
        foreach ($columns as $columnEntry) {
            if (empty($columnEntry['COMMENT'])) {
                throw new Varien_Db_Exception("Cannot create table without columns comments");
            }
        }
    ...
    protected function _getColumnsDefinition(Varien_Db_Ddl_Table $table)
    {
        ...
        $columns    = $table->getColumns();
        if (empty($columns)) {
            throw new Varien_Db_Exception('Table columns are not defined');
        }
    ...
    protected function _getColumnDefinition($options, $ddlType = null)
    {
        ...
        if (empty($ddlType) || !isset($this->_ddlColumnTypes[$ddlType])) {
            throw new Varien_Db_Exception('Invalid column definition data');
        }
        ...
        if ($ddlType == Varien_Db_Ddl_Table::TYPE_TIMESTAMP) {
            if ($cDefault === null) {
                $cDefault = new Magento_Db_Expr('NULL');
            } elseif ($cDefault == Varien_Db_Ddl_Table::TIMESTAMP_INIT) {
                $cDefault = new Magento_Db_Expr('CURRENT_TIMESTAMP');
            } else if ($cDefault == Varien_Db_Ddl_Table::TIMESTAMP_UPDATE) {
                $cDefault = new Magento_Db_Expr('0 ON UPDATE CURRENT_TIMESTAMP');
            } else if ($cDefault == Varien_Db_Ddl_Table::TIMESTAMP_INIT_UPDATE) {
                $cDefault = new Magento_Db_Expr('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP');
            } else if ($cNullable && !$cDefault) {
                $cDefault = new Magento_Db_Expr('NULL');
            } else {
                $cDefault = new Magento_Db_Expr('0');
            }
    ...
    public function truncateTable($tableName, $schemaName = null)
    {
        if (!$this->isTableExists($tableName, $schemaName)) {
            throw new Varien_Db_Exception(sprintf('Table "%s" is not exists', $tableName));
        }
    ...
    public function renameTable($oldTableName, $newTableName, $schemaName = null)
    {
        if (!$this->isTableExists($oldTableName, $schemaName)) {
            throw new Varien_Db_Exception(sprintf('Table "%s" is not exists', $oldTableName));
        }
        if ($this->isTableExists($newTableName, $schemaName)) {
            throw new Varien_Db_Exception(sprintf('Table "%s" already exists', $newTableName));
        }
    ...
    public function renameTablesBatch(array $tablePairs)
    {
        if (count($tablePairs) == 0) {
            throw new Varien_Db_Exception('Please provide tables for rename');
        }
     ...
     * @param string $schemaName
     * @return Zend_Db_Statement_Interface
     * @throws Varien_Db_Exception|Exception
     */
        ...
        foreach ($fields as $field) {
            if (!isset($columns[$field])) {
                $msg = sprintf('There is no field "%s" that you are trying to create an index on "%s"',
                    $field, $tableName);
                throw new Varien_Db_Exception($msg);
            }
    ...
    public function formatDate($date, $includeTime = true)
    {
        ...
        if ($date === null) {
            return new Magento_Db_Expr('NULL');
        }

        return new Magento_Db_Expr($this->quote($date));
    }
    ...
    {
        if ($value instanceof Magento_Db_Expr) {
            return $value;
        }
     ...
     *
     * @param Magento_Db_Expr|Varien_Db_Select|string $expression
     * @param string $true  true value
     * @param string $false false value
     */
    public function getCheckSql($expression, $true, $false)
    {
        if ($expression instanceof Magento_Db_Expr || $expression instanceof Varien_Db_Select) {
            $expression = sprintf("IF((%s), %s, %s)", $expression, $true, $false);
        } else {
            $expression = sprintf("IF(%s, %s, %s)", $expression, $true, $false);
        }

        return new Magento_Db_Expr($expression);
    }
    ...
    public function getConcatSql(array $data, $separator = null)
    {
        ...
        return new Magento_Db_Expr(sprintf($format, implode(', ', $data)));
    }
    ...
    {
        return new Magento_Db_Expr(sprintf('LENGTH(%s)', $string));
    }
    ...
    {
        return new Magento_Db_Expr(sprintf('LEAST(%s)', implode(', ', $data)));
    }
    ...
    {
        return new Magento_Db_Expr(sprintf('GREATEST(%s)', implode(', ', $data)));
    }
     ...
     *
     * @param Magento_Db_Expr|string $date   quoted field name or SQL statement
     * @param int $interval
     * @param string $unit
     * @return Magento_Db_Expr
     */
    public function getDateAddSql($date, $interval, $unit)
    {
        ...
        return new Magento_Db_Expr($expr);
    }
    ...
    {
        return new Magento_Db_Expr(sprintf('DATE(%s)', $date));
    }

    /**
     * Prepare substring sql function
     *
     * @param Magento_Db_Expr|string $stringExpression quoted field name or SQL statement
     * @param int|string|Magento_Db_Expr $pos
     * @param int|string|Magento_Db_Expr|null $len
     * @return Magento_Db_Expr
     */
    public function getSubstringSql($stringExpression, $pos, $len = null)
    {
        if (is_null($len)) {
            return new Magento_Db_Expr(sprintf('SUBSTRING(%s, %s)', $stringExpression, $pos));
        }
        return new Magento_Db_Expr(sprintf('SUBSTRING(%s, %s, %s)', $stringExpression, $pos, $len));
    }

    /**
     * Prepare standard deviation sql function
     *
     * @param Magento_Db_Expr|string $expressionField   quoted field name or SQL statement
     * @return Magento_Db_Expr
     */
    public function getStandardDeviationSql($expressionField)
    {
        return new Magento_Db_Expr(sprintf('STDDEV_SAMP(%s)', $expressionField));
    }
    ...
    public function selectsByRange($rangeField, Varien_Db_Select $select, $stepCount = 100)
    {
        ...
        $selectRange = $this->select()
            ->from(
                $tableName,
                array(
                    new Magento_Db_Expr('MIN(' . $this->quoteIdentifier($rangeField) . ') AS min'),
                    new Magento_Db_Expr('MAX(' . $this->quoteIdentifier($rangeField) . ') AS max'),
                )
            );
     ...
     *
     * @param string|Magento_Db_Expr $date
     * @throws Varien_Db_Exception
     * @return Magento_Db_Expr
     */
     ...
     *
     * @param int|Magento_Db_Expr $timestamp
     * @return mixed
     */
    ...
    public function updateFromSelect(Varien_Db_Select $select, $table)
    {
        ...
        foreach ($select->getPart(Varien_Db_Select::COLUMNS) as $columnEntry) {
            ...
            if (!$column instanceof Magento_Db_Expr && !empty($correlationName)) {
    ...
    {
        if ($field !== null) {
            $expression = new Magento_Db_Expr(sprintf('RAND() * %s', $this->quoteIdentifier($field)));
            $select->columns(array('mage_rand' => $expression));
            $spec = new Magento_Db_Expr('mage_rand');
        } else {
            $spec = new Magento_Db_Expr('RAND()');
        }
        $select->order($spec);
    ...
    protected function _prepareInsertData($row, &$bind)
    {
        if (is_array($row)) {
            ...
            $line = implode(', ', $line);
        } elseif ($row instanceof Magento_Db_Expr) {
            $line = $row->__toString();
        } else {
            $line = '?';
            $bind[] = $row;
     ...
     * In Mio at MySQL there was zero date used for datetime columns. However, zero date it is not supported across
     ...
     * the adapter.
     *
     * @param string|array|Magento_Db_Expr $ident The identifier.
