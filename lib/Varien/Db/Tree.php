...
Zend_Loader::loadClass('Magento_Db_Select');
...
class Varien_Db_Tree
{
    ...
    public function checkNodes() {
        ...
        $sql->from(array('t1'=>$this->_table), array('t1.'.$this->_id, new Magento_Db_Expr('COUNT(t1.'.$this->_id.') AS rep')))
        ->from(array('t2'=>$this->_table))
        ->from(array('t3'=>$this->_table), new Magento_Db_Expr('MAX(t3.'.$this->_right.') AS max_right'));
    ...
    protected function _addExtTablesToSelect(Magento_Db_Select &$select)
    {
    ...
    public function getChildren($ID, $start_level = 0, $end_level = 0)
    {
        ...
        $dbSelect = new Magento_Db_Select($this->_db);
