 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Varien_Http_UserAgent implements Serializable
{
    /**
     ...
     * Identified device
     *
     * @var Varien_Http_UserAgent_Device
     */
    protected $_device;
     ...
     *
     * @var Varien_Http_UserAgent_Storage
     */
    protected $_storage;
     ...
     * Constructor
     *
     * @param  null|array|Varien_Config|ArrayAccess $options
     * @return void
     */
     ...
     * Configure instance
     *
     * @param  array|Varien_Config|ArrayAccess $options
     * @return Varien_Http_UserAgent
     */
    public function setOptions($options)
    {
        ...
        if (!is_array($options)
            ...
            #require_once 'Varien/Http/UserAgent/Exception.php';
            throw new Varien_Http_UserAgent_Exception(sprintf(
                'Invalid argument; expected array, Varien_Config object, or object implementing ArrayAccess and Traversable; received %s',
    ...
    protected function _match($deviceClass)
    {
        ...
        if (!$r->implementsInterface('Varien_Http_UserAgent_Device')) {
            throw new Varien_Http_UserAgent_Exception(sprintf(
                'Invalid device class provided ("%s"); must implement Varien_Http_UserAgent_Device',
                $deviceClass
            ));
        }
     ...
     * @return string
     * @throws Varien_Loader_PluginLoader_Exception if unable to load UA device
     */
    protected function _getUserAgentDevice($browserType)
    {
        ...
        if (isset($this->_config[$browserType])
            ...
            if (is_array($deviceConfig) && isset($deviceConfig['classname'])) {
                ...
                $prefix = isset($deviceConfig['prefix']) ? $deviceConfig['prefix'] : 'Varien_Http_UserAgent';
     ...
     * @return Varien_Http_UserAgent_Storage
     */
     ...
     *
     * @param  Varien_Http_UserAgent_Storage $storage
     * @return Varien_Http_UserAgent
     */
    public function setStorage(Varien_Http_UserAgent_Storage $storage)
    {
        if ($this->_immutable) {
            #require_once 'Varien/Http/UserAgent/Exception.php';
            throw new Varien_Http_UserAgent_Exception(
    ...
    /**
     * Config parameters is an Array or a Varien_Config object
     *
     ...
     * capabilities.
     *
     * @return Varien_Http_UserAgent_Device $device
     */
    public function getDevice()
    {
        ...
        if (!$storage->isEmpty()) {
            ...
            // Varien_Http_UserAgent object is serialized in the session
            $object = $storage->read();
     ...
     * @return void
     * @throws Varien_Http_UserAgent_Exception on invalid parameter
     */
     ...
     * @param  string|Varien_Loader_PluginLoader $loader
     * @return Varien_Http_UserAgent
     */
    public function setPluginLoader($type, $loader)
    {
        ...
        if (is_string($loader)) {
            if (!class_exists($loader)) {
                ...
                Varien_Loader::loadClass($loader);
            }
            $loader = new $loader();
        ...
        if (!$loader instanceof Varien_Loader_PluginLoader) {
            #require_once 'Varien/Http/UserAgent/Exception.php';
            throw new Varien_Http_UserAgent_Exception(sprintf(
                'Expected an object extending Varien_Loader_PluginLoader; received %s',
                get_class($loader)
            ));
        }

        $basePrefix = 'Varien_Http_UserAgent_';
        $basePath   = 'Varien/Http/UserAgent/';
        switch ($type) {
            case 'storage':
     ...
     * @return Varien_Loader_PluginLoader
     */
    public function getPluginLoader($type)
    {
        $type = $this->_validateLoaderType($type);
        if (!isset($this->_loaders[$type])) {
            #require_once 'Varien/Loader/PluginLoader.php';
            $this->setPluginLoader($type, new Varien_Loader_PluginLoader());
        }
     ...
     * @param  string $type
     * @return string
     * @throws Varien_Http_UserAgent_Exception on invalid type
     */
     ...
     * user agent
     *
     * @return Varien_Http_UserAgent_Result
     */
