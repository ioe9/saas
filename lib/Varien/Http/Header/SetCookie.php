...
/**
 * @see Varien_Http_Header_Exception_InvalidArgumentException
 */
#require_once "Varien/Http/Header/Exception/InvalidArgumentException.php";

/**
 * @see Varien_Http_Header_Exception_RuntimeException
 */
#require_once "Varien/Http/Header/Exception/RuntimeException.php";
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Varien_Http_Header_SetCookie
{
     ...
     *
     * @static
     * @throws Varien_Http_Header_Exception_InvalidArgumentException
     * @param  $headerLine
    ...
    public static function fromString($headerLine, $bypassHeaderFieldName = false)
    {
        ...
        if (strtolower($name) !== 'set-cookie') {
            throw new Varien_Http_Header_Exception_InvalidArgumentException('Invalid header line for Set-Cookie string: "' . $name . '"');
        }
    ...
    /**
     * @throws Varien_Http_Header_Exception_RuntimeException
     * @return string
     */
    public function getFieldValue()
    {
        if ($this->getName() == '') {
            throw new Varien_Http_Header_Exception_RuntimeException('A cookie name is required to generate a field value for this cookie');
        }
    ...
    {
        if (preg_match("/[=,; \t\r\n\013\014]/", $name)) {
            throw new Varien_Http_Header_Exception_InvalidArgumentException("Cookie name cannot contain these characters: =,; \\t\\r\\n\\013\\014 ({$name})");
        }
    ...
    {
        if (!is_int($version)) {
            throw new Varien_Http_Header_Exception_InvalidArgumentException('Invalid Version number specified');
        }
    ...
    public function setMaxAge($maxAge)
    {
        if (!is_int($maxAge) || ($maxAge<0)) {
            throw new Varien_Http_Header_Exception_InvalidArgumentException('Invalid Max-Age number specified');
        }
        $this->maxAge = $maxAge;
    }
    ...
    public function setExpires($expires)
    {
        if (!empty($expires)) {
            if (is_string($expires)) {
                ...
                throw new Varien_Http_Header_Exception_InvalidArgumentException('Invalid expires time specified');
            }
    ...
    {
        Varien_Http_Header_HeaderValue::assertValid($domain);
        $this->domain = $domain;
        return $this;
    }
    ...
    {
        Varien_Http_Header_HeaderValue::assertValid($path);
        $this->path = $path;
        return $this;
    }
    ...
    public function toStringMultipleHeaders(array $headers)
    {
        ...
        foreach ($headers as $header) {
            if (!$header instanceof Varien_Http_Header_SetCookie) {
                throw new Varien_Http_Header_Exception_RuntimeException(
