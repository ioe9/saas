...
/**
 * @see Varien_Http_Cookie
 */
#require_once "Varien/Http/Cookie.php";
/**
...
/**
 * A Varien_Http_CookieJar object is designed to contain and maintain HTTP cookies, and should
 * be used along with Varien_Http_Client in order to manage cookies across HTTP requests and
 * responses.
 *
 * The class contains an array of Varien_Http_Cookie objects. Cookies can be added to the jar
 ...
 * can be returned either in their native form (as Varien_Http_Cookie objects) or as strings -
 ...
 * (by passing Varien_Http_CookieJar::COOKIE_STRING_ARRAY) or one unified string for all cookies
 * (by passing Varien_Http_CookieJar::COOKIE_STRING_CONCAT).
 *
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Varien_Http_CookieJar implements Countable, IteratorAggregate
{
    /**
     * Return cookie(s) as a Varien_Http_Cookie object
     *
     */
    ...
    /**
     * Add a cookie to the jar. Cookie should be passed either as a Varien_Http_Cookie object
     * or as a string - in which case an object is created from the string.
     *
     * @param Varien_Http_Cookie|string $cookie
     * @param Varien_Uri_Http|string    $ref_uri Optional reference URI (for domain, path, secure)
     * @param boolean $encodeValue
     */
    public function addCookie($cookie, $ref_uri = null, $encodeValue = true)
    {
        if (is_string($cookie)) {
            $cookie = Varien_Http_Cookie::fromString($cookie, $ref_uri, $encodeValue);
        }
        ...
        if ($cookie instanceof Varien_Http_Cookie) {
            ...
            throw new Varien_Http_Exception('Supplient argument is not a valid cookie string or object');
        }
    }
     ...
     * to the cookie jar.
     *
     * @param Varien_Http_Response $response
     * @param Varien_Uri_Http|string $ref_uri Requested URI
     * @param boolean $encodeValue
     */
    public function addCookiesFromResponse($response, $ref_uri, $encodeValue = true)
    {
        if (! $response instanceof Varien_Http_Response) {
            #require_once 'Varien/Http/Exception.php';
            throw new Varien_Http_Exception('$response is expected to be a Response object, ' .
     ...
     *
     * @param int $ret_as Whether to return cookies as objects of Varien_Http_Cookie or as strings
     * @return array|string
     */
     ...
     *
     * @param string|Varien_Uri_Http $uri URI to check against (secure, domain, path)
    ...
    {
        if (is_string($uri)) $uri = Varien_Uri::factory($uri);
        if (! $uri instanceof Varien_Uri_Http) {
            #require_once 'Varien/Http/Exception.php';
            throw new Varien_Http_Exception("Invalid URI string or object passed");
        }
     ...
     *
     * @param Varien_Uri_Http|string $uri The uri (domain and path) to match
     * @param string $cookie_name The cookie's name
     * @param int $ret_as Whether to return cookies as objects of Varien_Http_Cookie or as strings
     * @return Varien_Http_Cookie|string
     */
    public function getCookie($uri, $cookie_name, $ret_as = self::COOKIE_OBJECT)
    {
        ...
        if (! $uri instanceof Varien_Uri_Http) {
            #require_once 'Varien/Http/Exception.php';
            throw new Varien_Http_Exception('Invalid URI specified');
        }
        ...
        if (isset($this->cookies[$uri->getHost()][$path][$cookie_name])) {
            ...
            switch ($ret_as) {
                ...
                default:
                    #require_once 'Varien/Http/Exception.php';
                    throw new Varien_Http_Exception("Invalid value passed for \$ret_as: {$ret_as}");
                    break;
            }
        } else {
            return false;
     ...
     *
     * @param Varien_Http_Cookie|array $ptr
    ...
    protected function _flattenCookiesArray($ptr, $ret_as = self::COOKIE_OBJECT) {
        if (is_array($ptr)) {
            ...
            return $ret;
        } elseif ($ptr instanceof Varien_Http_Cookie) {
            switch ($ret_as) {
                case self::COOKIE_STRING_ARRAY:
    ...
    protected function _matchDomain($domain)
    {
        ...
        foreach (array_keys($this->cookies) as $cdom) {
            if (Varien_Http_Cookie::matchCookieDomain($cdom, $domain)) {
    ...
    protected function _matchPath($domains, $path)
    {
        ...
        foreach ($domains as $dom => $paths_array) {
            foreach (array_keys($paths_array) as $cpath) {
                if (Varien_Http_Cookie::matchCookiePath($cpath, $path)) {
                    if (! isset($ret[$dom])) {
                        $ret[$dom] = array();
                    }
     ...
     * of the cookie.
     *
     * @param Varien_Http_Response $response HTTP Response object
     * @param Varien_Uri_Http|string $uri The requested URI
     * @return Varien_Http_CookieJar
     * @todo Add the $uri functionality.
     */
    public static function fromResponse(Varien_Http_Response $response, $ref_uri)
    {
        $jar = new self();
