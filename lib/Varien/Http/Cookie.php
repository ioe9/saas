...
/**
 * Varien_Http_Cookie is a class describing an HTTP cookie and all it's parameters.
 *
 * Varien_Http_Cookie is a class describing an HTTP cookie and all it's parameters. The
...
class Varien_Http_Cookie
{
    ...
    public function __construct($name, $value, $domain, $expires = null, $path = null, $secure = false)
    {
        if (preg_match("/[=,; \t\r\n\013\014]/", $name)) {
            ...
            throw new Varien_Http_Exception("Cookie name cannot contain these characters: =,; \\t\\r\\n\\013\\014 ({$name})");
        }

        if (! $this->name = (string) $name) {
            #require_once 'Varien/Http/Exception.php';
            throw new Varien_Http_Exception('Cookies must have a name');
        }

        if (! $this->domain = (string) $domain) {
            #require_once 'Varien/Http/Exception.php';
            throw new Varien_Http_Exception('Cookies must have a domain');
        }
    ...
    {
        if (is_string ($uri)) {
            $uri = Varien_Uri_Http::factory($uri);
        }

        // Make sure we have a valid Varien_Uri_Http object
        if (! ($uri->valid() && ($uri->getScheme() == 'http' || $uri->getScheme() =='https'))) {
            #require_once 'Varien/Http/Exception.php';
            throw new Varien_Http_Exception('Passed URI is not a valid HTTP or HTTPS URI');
        }
     ...
     * @param Varien_Uri_Http|string $refUri Reference URI for default values (domain, path)
     * @param boolean $encodeValue Whether or not the cookie's value should be
     *                             passed through urlencode/urldecode
     * @return Varien_Http_Cookie A new Varien_Http_Cookie object or false on failure.
     */
    public static function fromString($cookieStr, $refUri = null, $encodeValue = true)
    {
        // Set default values
        if (is_string($refUri)) {
            $refUri = Varien_Uri_Http::factory($refUri);
        }
        ...
        if ($refUri instanceof Varien_Uri_Http) {
                ...
                switch (strtolower($k))    {
                    case 'expires':
                        if(($expires = strtotime($v)) === false) {
                             ...
                             * the maximum for 32-bit signed integer. Varien_Date
                             * can get around that limit.
                             *
                             * @see Varien_Date
                             */
                            #require_once 'Varien/Date.php';

                            $expireDate = new Varien_Date($v);
     ...
     *
     * Used by Varien_Http_Cookie and Varien_Http_CookieJar for cookie matching
     *
    ...
    public static function matchCookieDomain($cookieDomain, $host)
    {
        if (! $cookieDomain) {
            #require_once 'Varien/Http/Exception.php';
            throw new Varien_Http_Exception("\$cookieDomain is expected to be a cookie domain");
        }

        if (! $host) {
            #require_once 'Varien/Http/Exception.php';
            throw new Varien_Http_Exception("\$host is expected to be a host name");
        }
    ...
    public static function matchCookiePath($cookiePath, $path)
    {
        if (! $cookiePath) {
            #require_once 'Varien/Http/Exception.php';
            throw new Varien_Http_Exception("\$cookiePath is expected to be a cookie path");
        }

        if (! $path) {
            #require_once 'Varien/Http/Exception.php';
            throw new Varien_Http_Exception("\$path is expected to be a host name");
        }
