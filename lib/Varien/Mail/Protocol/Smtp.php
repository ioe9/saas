<?php
class Varien_Mail_Protocol_Smtp extends Varien_Mail_Protocol_Abstract
{
    /**
    ...
    public function __construct($host = '127.0.0.1', $port = null, array $config = array())
    {
        ...
        if (isset($config['ssl'])) {
            switch (strtolower($config['ssl'])) {
                ...
                default:
                    /**
                     * @see Varien_Mail_Protocol_Exception
                     */
                    #require_once 'Zend/Mail/Protocol/Exception.php';
                    throw new Varien_Mail_Protocol_Exception($config['ssl'] . ' is unsupported SSL type');
                    break;
            }
        }
    ...
    public function helo($host = '127.0.0.1')
    {
        ...
        if ($this->_sess === true) {
             ...
             */
            #require_once 'Zend/Mail/Protocol/Exception.php';
            throw new Varien_Mail_Protocol_Exception('Cannot issue HELO to existing session');
        }
        ...
        if ($this->_secure == 'tls') {
            ...
            if (!stream_socket_enable_crypto($this->_socket, true, STREAM_CRYPTO_METHOD_TLS_CLIENT)) {
                 ...
                 */
                #require_once 'Zend/Mail/Protocol/Exception.php';
                throw new Varien_Mail_Protocol_Exception('Unable to connect via TLS');
            }
            $this->_ehlo($host);
        }
    ...
    public function mail($from)
    {
        if ($this->_sess !== true) {
             ...
             */
            #require_once 'Zend/Mail/Protocol/Exception.php';
            throw new Varien_Mail_Protocol_Exception('A valid session has not been started');
        }
    ...
    public function rcpt($to)
    {
        if ($this->_mail !== true) {
             ...
             */
            #require_once 'Zend/Mail/Protocol/Exception.php';
            throw new Varien_Mail_Protocol_Exception('No sender reverse path has been supplied');
        }
    ...
    public function data($data)
    {
        ...
        if ($this->_rcpt !== true) {
             ...
             */
            #require_once 'Zend/Mail/Protocol/Exception.php';
            throw new Varien_Mail_Protocol_Exception('No recipient forward path has been supplied');
        }

        $this->_send('DATA');
        $this->_expect(354, 120); // Timeout set for 2 minutes as per RFC 2821 4.5.3.2

        foreach (explode(Varien_Mime::LINEEND, $data) as $line) {
            if (strpos($line, '.') === 0) {
     ...
     *
     * Not used by Varien_Mail, could be used to keep a connection alive or check if it is still open.
     *
     * @return void
     */
    ...
    public function auth()
    {
        if ($this->_auth === true) {
             ...
             */
            #require_once 'Zend/Mail/Protocol/Exception.php';
            throw new Varien_Mail_Protocol_Exception('Already authenticated for this session');
        }
    }
