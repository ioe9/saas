 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class Varien_Mail_Transport_Abstract
{
    /**
     * Mail body
     * @var string
     ...
     * on the mail parts present in the {@link $_mail Varien_Mail object} present.
     *
        ...
        if (null !== $boundary) {
            ...
            if (!$type) {
                if ($this->_mail->hasAttachments) {
                    $type = Varien_Mime::MULTIPART_MIXED;
                } elseif ($this->_mail->getBodyText() && $this->_mail->getBodyHtml()) {
                    $type = Varien_Mime::MULTIPART_ALTERNATIVE;
                } else {
                    $type = Varien_Mime::MULTIPART_MIXED;
                }
            }
     ...
     * @access protected
     * @return void
     * @throws Varien_Mail_Transport_Exception if any header lines exceed 998
     * characters
     */
    protected function _prepareHeaders($headers)
    {
        if (!$this->_mail) {
            ...
            throw new Varien_Mail_Transport_Exception('Missing Varien_Mail object in _mail property');
        }
        ...
        if (!$sane) {
             ...
             */
            #require_once 'Zend/Mail/Transport/Exception.php';
            throw new Varien_Mail_Exception('At least one mail header line is too long');
        }
    }
     ...
     * multipart/alternative Varien_Mime_Part containing the headers and contents
     * of each. Otherwise, uses whichever of the text or HTML parts present.
     *
     * The content part is then prepended to the list of Varien_Mime_Parts for
     * this message.
     *
     * @return void
     */
    protected function _buildBody()
    {
            ...
            && ($html = $this->_mail->getBodyHtml()))
        {
            ...
            $mp->type     = Varien_Mime::MULTIPART_ALTERNATIVE;
        ...
        if (!$body) {
             ...
             */
            #require_once 'Zend/Mail/Transport/Exception.php';
            throw new Varien_Mail_Transport_Exception('No body specified');
        }
        ...
        foreach ($headers as $header) {
            // Headers in Varien_Mime_Part are kept as arrays with two elements, a
            // key and a value
     ...
     *
     * @param  Varien_Mail $mail
     * @access public
     * @return void
     * @throws Varien_Mail_Transport_Exception if mail is empty
     */
    public function send(Varien_Mail $mail)
    {
        ...
        if ($count < 1) {
             ...
             */
            #require_once 'Zend/Mail/Transport/Exception.php';
            throw new Varien_Mail_Transport_Exception('Empty mail cannot be sent');
        }

        if ($count > 1) {
            // Multipart message; create new MIME object and boundary
            $mime     = new Varien_Mime($this->_mail->getMimeBoundary());
        ...
        // Create message body
        // This is done so that the same Varien_Mail object can be used in
        // multiple transports
        $message = new Varien_Mime_Message();
