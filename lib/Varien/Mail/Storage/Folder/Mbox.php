 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Varien_Mail_Storage_Folder_Mbox extends Varien_Mail_Storage_Mbox implements Varien_Mail_Storage_Folder_Interface
{
    /**
     * Varien_Mail_Storage_Folder root folder for folder structure
     * @var Varien_Mail_Storage_Folder
     */
     ...
     *   - filename use Varien_Mail_Storage_Mbox for a single file
    ...
    public function __construct($params)
    {
        ...
        if (isset($params->filename)) {
            ...
            throw new Varien_Mail_Storage_Exception('use Varien_Mail_Storage_Mbox for a single file');
        }
        ...
        if (!isset($params->dirname) || !is_dir($params->dirname)) {
             ...
             */
            #require_once 'Zend/Mail/Storage/Exception.php';
            throw new Varien_Mail_Storage_Exception('no valid dirname given in params');
        }
     ...
     *
     * Result is save in Varien_Mail_Storage_Folder instances with the root in $this->_rootFolder.
     * $parentFolder and $parentGlobalName are only used internally for recursion.
     *
     * @param string $currentDir call with root dir, also used for recursion.
     * @param Varien_Mail_Storage_Folder|null $parentFolder used for recursion
    ...
    {
        if (!$parentFolder) {
            $this->_rootFolder = new Varien_Mail_Storage_Folder('/', '/', false);
        ...
        if (!$dh) {
             ...
             */
            #require_once 'Zend/Mail/Storage/Exception.php';
            throw new Varien_Mail_Storage_Exception("can't read dir $currentDir");
        }
        while (($entry = readdir($dh)) !== false) {
            ...
            if (is_file($absoluteEntry) && $this->_isMboxFile($absoluteEntry)) {
                $parentFolder->$entry = new Varien_Mail_Storage_Folder($entry, $globalName);
                continue;
            }
            if (!is_dir($absoluteEntry) /* || $entry == '.' || $entry == '..' */) {
                continue;
            }
            $folder = new Varien_Mail_Storage_Folder($entry, $globalName, false);
    ...
    public function getFolders($rootFolder = null)
    {
        ...
        if ($currentFolder->getGlobalName() != DIRECTORY_SEPARATOR . trim($rootFolder, DIRECTORY_SEPARATOR)) {
             ...
             */
            #require_once 'Zend/Mail/Storage/Exception.php';
            throw new Varien_Mail_Storage_Exception("folder $rootFolder not found");
        }
    ...
    public function selectFolder($globalName)
    {
        ...
        try {
            ...
            if (!$folder->isSelectable()) {
                 ...
                 */
                #require_once 'Zend/Mail/Storage/Exception.php';
                throw new Varien_Mail_Storage_Exception("{$this->_currentFolder} is not selectable", 0, $e);
            }
             ...
             */
            #require_once 'Zend/Mail/Storage/Exception.php';
            throw new Varien_Mail_Storage_Exception('seems like the mbox file has vanished, I\'ve rebuild the ' .
