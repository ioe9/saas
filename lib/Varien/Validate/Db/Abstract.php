 ...
 * @category   Zend
 * @package    Varien_Validate
 * @uses       Varien_Validate_Abstract
 * @copyright  Copyright (c) 2005-2015 Zend Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class Varien_Validate_Db_Abstract extends Varien_Validate_Abstract
{
    /**
     * Error constants
     */
    ...
    /**
     * Provides basic configuration for use with Varien_Validate_Db Validators
    ...
    {
        if ($options instanceof Magento_Db_Select) {
        ...
        if (!array_key_exists('table', $options) && !array_key_exists('schema', $options)) {
            ...
            throw new Varien_Validate_Exception('Table or Schema option missing!');
        }

        if (!array_key_exists('field', $options)) {
            #require_once 'Zend/Validate/Exception.php';
            throw new Varien_Validate_Exception('Field option missing!');
        }
    ...
    public function getAdapter()
    {
        ...
        if ($this->_adapter === null) {
            ...
            if (null === $this->_adapter) {
                #require_once 'Zend/Validate/Exception.php';
                throw new Varien_Validate_Exception('No database adapter present');
            }
        }
     ...
     * @throws Varien_Validate_Exception
     * @return Varien_Validate_Db_Abstract
     */
    public function setAdapter($adapter)
    {
        if (!($adapter instanceof Zend_Db_Adapter_Abstract)) {
            #require_once 'Zend/Validate/Exception.php';
            throw new Varien_Validate_Exception('Adapter option must be a database adapter!');
        }
    ...
    public function setSelect($select)
    {
        if (!$select instanceof Magento_Db_Select) {
            throw new Varien_Validate_Exception('Select option must be a valid ' .
                                              'Magento_Db_Select object');
        }
        $this->_select = $select;
        return $this;
    }
     ...
     *
     * @return Magento_Db_Select The Select object which will be used
     */
    public function getSelect()
    {
        if (null === $this->_select) {
             ...
             * Build select object
             */
            $select = new Magento_Db_Select($db);
