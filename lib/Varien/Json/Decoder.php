...
class Varien_Json_Decoder
{
     ...
     * {@link Varien_Json::TYPE_ARRAY} and {@link Varien_Json::TYPE_OBJECT} for
     * valid values
     * @return void
     */
    protected function __construct($source, $decodeType)
    {
        ...
        if (!in_array($decodeType, array(Varien_Json::TYPE_ARRAY, Varien_Json::TYPE_OBJECT)))
        {
            $decodeType = Varien_Json::TYPE_ARRAY;
        }
     ...
     * return a StdClass object instead, pass {@link Varien_Json::TYPE_OBJECT} to
     * the $objectDecodeType parameter.
     *
     * Throws a Varien_Json_Exception if the source string is null.
     *
     * @static
     * @access public
     * @param string $source String to be decoded
     * @param int $objectDecodeType How objects should be decoded; should be
     * either or {@link Varien_Json::TYPE_ARRAY} or
     * {@link Varien_Json::TYPE_OBJECT}; defaults to TYPE_ARRAY
     * @return mixed
     * @throws Varien_Json_Exception
     */
    public static function decode($source = null, $objectDecodeType = Varien_Json::TYPE_ARRAY)
    {
        if (null === $source) {
            #require_once 'Zend/Json/Exception.php';
            throw new Varien_Json_Exception('Must specify JSON encoded source for decoding');
        } elseif (!is_string($source)) {
            #require_once 'Zend/Json/Exception.php';
            throw new Varien_Json_Exception('Can only decode JSON encoded strings');
        }
     ...
     *
     * If Varien_Json_Encoder was used to encode the original object then
    ...
    protected function _decodeObject()
    {
        ...
        while ($tok && $tok != self::RBRACE) {
            if ($tok != self::DATUM || ! is_string($this->_tokenValue)) {
                #require_once 'Zend/Json/Exception.php';
                throw new Varien_Json_Exception('Missing key in object encoding: ' . $this->_source);
            }
            ...
            if ($tok != self::COLON) {
                #require_once 'Zend/Json/Exception.php';
                throw new Varien_Json_Exception('Missing ":" in object encoding: ' . $this->_source);
            }
    ...
    protected function _decodeArray()
    {
        ...
        while ($tok && $tok != self::RBRACKET) {
            ...
            if ($tok != self::COMMA) {
                #require_once 'Zend/Json/Exception.php';
                throw new Varien_Json_Exception('Missing "," in array encoding: ' . $this->_source);
            }
    ...
    protected function _getNextToken()
    {
                ...
                do {
                    ...
                    if ($chr == '\\') {
                        ...
                        switch ($chr) {
                            ...
                            default:
                                #require_once 'Zend/Json/Exception.php';
                                throw new Varien_Json_Exception("Illegal escape "
                                    .  "sequence '" . $chr . "'");
                        }
                    } elseif($chr == '"') {
                        break;
        ...
        if ($chr == '-' || $chr == '.' || ($chr >= '0' && $chr <= '9')) {
            if (preg_match('/-?([0-9])*(\.[0-9]*)?((e|E)((-|\+)?)[0-9]+)?/s',
                ...
                if (is_numeric($datum)) {
                    if (preg_match('/^0\d+$/', $datum)) {
                        #require_once 'Zend/Json/Exception.php';
                        throw new Varien_Json_Exception("Octal notation not supported by JSON (value: $datum)");
                    } else {
                        $val  = intval($datum);
                        $fVal = floatval($datum);
                    ...
                    #require_once 'Zend/Json/Exception.php';
                    throw new Varien_Json_Exception("Illegal number format: $datum");
                }
            ...
            #require_once 'Zend/Json/Exception.php';
            throw new Varien_Json_Exception('Illegal Token');
        }
