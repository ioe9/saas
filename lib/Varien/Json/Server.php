 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Varien_Json_Server extends Varien_Server_Abstract
{
    /**#@+
     * Version Constants
     */
    ...
    /**
     * Request object
     * @var Varien_Json_Server_Request
     */
    protected $_request;

    /**
     * Response object
     * @var Varien_Json_Server_Response
     */
    protected $_response;

    /**
     * SMD object
     * @var Varien_Json_Server_Smd
     */
     ...
     * @return Varien_Json_Server
     */
    public function addFunction($function, $namespace = '')
    {
        if (!is_string($function) && (!is_array($function) || (2 > count($function)))) {
            #require_once 'Zend/Json/Server/Exception.php';
            throw new Varien_Json_Server_Exception('Unable to attach function; invalid');
        }

        if (!is_callable($function)) {
            #require_once 'Zend/Json/Server/Exception.php';
            throw new Varien_Json_Server_Exception('Unable to attach function; does not exist');
        }
        ...
        if (is_string($function)) {
            $method = Varien_Server_Reflection::reflectFunction($function, $argv, $namespace);
        } else {
            $class  = array_shift($function);
            $action = array_shift($function);
            $reflection = Varien_Server_Reflection::reflectClass($class, $argv, $namespace);
    ...
    public function fault($fault = null, $code = 404, $data = null)
    {
        ...
        $error = new Varien_Json_Server_Error($fault, $code, $data);
     ...
     * Handle request
     *
     * @param  Varien_Json_Server_Request $request
     * @return null|Varien_Json_Server_Response
     */
    public function handle($request = false)
    {
        if ((false !== $request) && (!$request instanceof Varien_Json_Server_Request)) {
            #require_once 'Zend/Json/Server/Exception.php';
            throw new Varien_Json_Server_Exception('Invalid request type provided; cannot handle');
        } elseif ($request) {
     ...
     *
     * @param  array|Varien_Server_Definition $definition
     * @return void
     */
    public function loadFunctions($definition)
    {
        if (!is_array($definition) && (!$definition instanceof Varien_Server_Definition)) {
            #require_once 'Zend/Json/Server/Exception.php';
            throw new Varien_Json_Server_Exception('Invalid definition provided to loadFunctions()');
        }
     ...
     * @return Varien_Json_Server
     */
    public function setRequest(Varien_Json_Server_Request $request)
    {
     ...
     *
     * @return Varien_Json_Server_Request
     */
    public function getRequest()
    {
        if (null === ($request = $this->_request)) {
            #require_once 'Zend/Json/Server/Request/Http.php';
            $this->setRequest(new Varien_Json_Server_Request_Http());
        }
     ...
     * Set response object
     *
     * @param  Varien_Json_Server_Response $response
     * @return Varien_Json_Server
     */
    public function setResponse(Varien_Json_Server_Response $response)
    {
     ...
     * Get response object
     *
     * @return Varien_Json_Server_Response
     */
    public function getResponse()
    {
        if (null === ($response = $this->_response)) {
            #require_once 'Zend/Json/Server/Response/Http.php';
            $this->setResponse(new Varien_Json_Server_Response_Http());
        }
     ...
     * Retrieve SMD object
     *
     * @return Varien_Json_Server_Smd
     */
    public function getServiceMap()
    {
        if (null === $this->_serviceMap) {
            #require_once 'Zend/Json/Server/Smd.php';
            $this->_serviceMap = new Varien_Json_Server_Smd();
        }
     ...
     *
     * @param  Varien_Server_Reflection_Function $method
     * @return void
     */
    protected function _addMethodServiceMap(Varien_Server_Method_Definition $method)
    {
        $serviceInfo = array(
            'name'   => $method->getName(),
     ...
     *
     * @param  Varien_Server_Reflection_Function_Abstract $method
     * @return string|array
     */
    protected function _getParams(Varien_Server_Method_Definition $method)
    {
        $params = array();
     ...
     * @return string|array
     */
    protected function _getReturnType(Varien_Server_Method_Definition $method)
    {
        $return = array();
        foreach ($method->getPrototypes() as $prototype) {
    ...
    protected function _getSmdMethods()
    {
        if (null === $this->_smdMethods) {
            ...
            #require_once 'Zend/Json/Server/Smd.php';
            $methods = get_class_methods('Varien_Json_Server_Smd');
        ...
        if ( is_string( key( $params ) ) ) {
            ...
            foreach( $reflection->getParameters() as $refParam ) {
                if( isset( $params[ $refParam->getName() ] ) ) {
                    ...
                    throw new Varien_Server_Exception(
