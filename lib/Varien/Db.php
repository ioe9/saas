...
class Varien_Db
{
    ...
    /**
     * Use the PROFILER constant in the config of a Varien_Db_Adapter.
     */
    const PROFILER = 'profiler';

    /**
     * Use the CASE_FOLDING constant in the config of a Varien_Db_Adapter.
     */
    const CASE_FOLDING = 'caseFolding';

    /**
     * Use the FETCH_MODE constant in the config of a Varien_Db_Adapter.
     */
    const FETCH_MODE = 'fetchMode';

    /**
     * Use the AUTO_QUOTE_IDENTIFIERS constant in the config of a Varien_Db_Adapter.
     */
    const AUTO_QUOTE_IDENTIFIERS = 'autoQuoteIdentifiers';

    /**
     * Use the ALLOW_SERIALIZATION constant in the config of a Varien_Db_Adapter.
     */
    const ALLOW_SERIALIZATION = 'allowSerialization';

    /**
     * Use the AUTO_RECONNECT_ON_UNSERIALIZE constant in the config of a Varien_Db_Adapter.
     */
    ...
    /**
     * Factory for Varien_Db_Adapter_Abstract classes.
     *
     * First argument may be a string containing the base of the adapter class
     * name, e.g. 'Mysqli' corresponds to class Varien_Db_Adapter_Mysqli.  This
     ...
     *
     * First argument may alternatively be an object of type Varien_Config.
     ...
     *
     * If the first argument is of type Varien_Config, it is assumed to contain
     * all parameters, and the second argument is ignored.
     *
     * @param  mixed $adapter String name of base adapter class, or Varien_Config object.
     * @param  mixed $config  OPTIONAL; an array or Varien_Config object with adapter parameters.
     * @return Varien_Db_Adapter_Abstract
    ...
    public static function factory($adapter, $config = array())
    {
        ...
        /*
         * Convert Varien_Config argument to plain string
         * adapter name and separate config object.
         */
        if ($adapter instanceof Varien_Config) {
        ...
        if (!is_array($config)) {
            ...
            throw new Varien_Db_Exception('Adapter parameters must be in an array or a Varien_Config object');
        }
        ...
        if (!is_string($adapter) || empty($adapter)) {
             ...
             */
            #require_once 'Zend/Db/Exception.php';
            throw new Varien_Db_Exception('Adapter name must be specified in a string');
        }

        /*
         * Form full adapter class name
         */
        $adapterNamespace = 'Varien_Db_Adapter';
         ...
         */
        if (! $dbAdapter instanceof Varien_Db_Adapter_Abstract) {
            /**
             * @see Varien_Db_Exception
             */
            #require_once 'Zend/Db/Exception.php';
            throw new Varien_Db_Exception("Adapter class '$adapterName' does not extend Varien_Db_Adapter_Abstract");
        }
