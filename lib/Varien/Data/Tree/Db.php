...
class Varien_Data_Tree_Db extends Varien_Data_Tree
{
    ...
    public function appendChild($data=array(), $parentNode, $prevNode=null)
    {
        ...
        $orderSelect->from($this->_table, new Magento_Db_Expr('MAX('.$this->_conn->quoteIdentifier($this->_orderField).')'))
    ...
    public function moveNodeTo($node, $parentNode, $prevNode=null)
    {
        ...
        $dataReorderNew = array(
            $this->_orderField => new Magento_Db_Expr($this->_conn->quoteIdentifier($this->_orderField).'+1')
        );
