...
class Varien_Data_Tree_Dbp extends Varien_Data_Tree
{
    ...
    public function load($parentNode=null, $recursionLevel = 0)
    {
        ...
        if (!$this->_loaded) {
			...
			//echo "<xmp>";var_dump($childrenItems);echo "</xmp>";
    ...
    public function move($node, $newParent, $prevNode = null)
    {
        ...
        $data = array(
            $this->_levelField => new Magento_Db_Expr("{$this->_levelField} + '{$levelDisposition}'"),
            $this->_pathField  => new Magento_Db_Expr("CONCAT('$newPath', RIGHT($this->_pathField, LENGTH($this->_pathField) - {$oldPathLength}))")
        );
        $condition = $this->_conn->quoteInto("$this->_pathField REGEXP ?", "^$oldPath(/|$)");

        $this->_conn->beginTransaction();

        $reorderData = array($this->_orderField => new Magento_Db_Expr("$this->_orderField + 1"));
        try {
            if ($prevNode && $prevNode->getId()) {
                ...
                $select = $this->_conn->select()
                    ->from($this->_table, new Magento_Db_Expr("MIN({$this->_orderField})"))
