...
class Varien_Data_Collection_Db extends Varien_Data_Collection
{
    ...
    {
        if (!$conn instanceof Varien_Db_Adapter_Abstract) {
            throw new Varien_Exception('dbModel read resource does not implement Varien_Db_Adapter_Abstract');
        }
    ...
    /**
     * Get Magento_Db_Select instance
     *
    ...
    public function getSelectCountSql()
    {
        ...
        $countSelect->reset(Magento_Db_Select::ORDER);
     ...
     * @return  string || Magento_Db_Select
     */
    ...
    public function addFieldToFilter($field, $condition = null)
    {
        if (!is_array($field)) {
            ...
            $resultCondition = '(' . join(') ' . Magento_Db_Select::SQL_OR . ' (', $conditions) . ')';
        }
    ...
    protected function _renderOrders()
    {
        if (!$this->_isOrdersRendered) {
            foreach ($this->_orders as $field => $direction) {
                $this->_select->order(new Magento_Db_Expr($field . ' ' . $direction));
             }
