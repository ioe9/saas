...
class Varien_Data_Form_Element_Date extends Varien_Data_Form_Element_Abstract
{
    ...
    /**
     * Set date value
     * If Varien_Date instance is provided instead of value, other params will be ignored.
     * Format and locale must be compatible with Varien_Date
     *
     * @param mixed $value
     * @param string $format
     * @param string $locale
    ...
    public function setValue($value, $format = null, $locale = null)
    {
        ...
        if ($value instanceof Varien_Date) {
            $this->_value = $value;
            return $this;
        }
        if (preg_match('/^[0-9]+$/', $value)) {
            $this->_value = new Varien_Date($this->_toTimestamp($value));
            //$this->_value = new Varien_Date((int)value);
            return $this;
        }
        ...
        try {
            $this->_value = new Varien_Date($value, $format, $locale);
        }
        catch (Exception $e) {
            $this->_value = '';
        }
     ...
     *
     * @param string $format (compatible with Varien_Date)
     * @return string
     */
     ...
     * - the value must be instantiated (Varien_Date)
     * - output format must be set (compatible with Varien_Date)
     *
     * @return string
     */
