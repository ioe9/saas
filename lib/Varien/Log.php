...
/**
 * Convenience methods for log [@see Varien_Log::__call()]:
 *
...
class Varien_Log
{
    ...
    /**
     * @var array of Varien_Log_Writer_Abstract
     */
     ...
     * @var string
     */
    protected $_defaultWriterNamespace = 'Varien_Log_Writer';

    /**
     *
     * @var string
     */
    protected $_defaultFilterNamespace = 'Varien_Log_Filter';

    /**
     *
     * @var string
     */
    protected $_defaultFormatterNamespace = 'Varien_Log_Formatter';
     ...
     *
     * @param Varien_Log_Writer_Abstract|null  $writer  default writer
     */
    public function __construct(Varien_Log_Writer_Abstract $writer = null)
    {
     ...
     *
     * @param  array|Varien_Config Array or instance of Varien_Config
     * @return Varien_Log
    ...
    static public function factory($config = array())
    {
        ...
        if (!$log instanceof Varien_Log) {
            /** @see Varien_Log_Exception */
            #require_once 'Varien/Log/Exception.php';
            throw new Varien_Log_Exception('Passed className does not belong to a descendant of Varien_Log');
        }
    ...
    protected function _constructWriterFromConfig($config)
    {
        ...
        if (!$writer instanceof Varien_Log_Writer_Abstract) {
            $writerName = is_object($writer)
                        ? get_class($writer)
                        : 'The specified writer';
            /** @see Varien_Log_Exception */
            #require_once 'Varien/Log/Exception.php';
            throw new Varien_Log_Exception("{$writerName} does not extend Varien_Log_Writer_Abstract!");
        }
    ...
    /**
     * Construct filter object from configuration array or Varien_Config object
     *
     * @param  array|Varien_Config $config Varien_Config or Array
     * @return Varien_Log_Filter_Interface
     * @throws Varien_Log_Exception
     */
    protected function _constructFilterFromConfig($config)
    {
        ...
        if (!$filter instanceof Varien_Log_Filter_Interface) {
            ...
            /** @see Varien_Log_Exception */
            #require_once 'Varien/Log/Exception.php';
            throw new Varien_Log_Exception("{$filterName} does not implement Varien_Log_Filter_Interface");
        }
   ...
   /**
    * Construct formatter object from configuration array or Varien_Config object
    *
    * @param  array|Varien_Config $config Varien_Config or Array
    * @return Varien_Log_Formatter_Interface
    * @throws Varien_Log_Exception
    */
    protected function _constructFormatterFromConfig($config)
    {
        ...
        if (!$formatter instanceof Varien_Log_Formatter_Interface) {
             $formatterName = is_object($formatter)
                         ? get_class($formatter)
                         : 'The specified formatter';
            /** @see Varien_Log_Exception */
            #require_once 'Varien/Log/Exception.php';
            throw new Varien_Log_Exception($formatterName . ' does not implement Varien_Log_Formatter_Interface');
        }
     ...
     * @param mixed $config Varien_Config or Array
    ...
    protected function _constructFromConfig($type, $config, $namespace)
    {
        ...
        if (!class_exists($className)) {
            ...
            Varien_Loader::loadClass($className);
        }

        $reflection = new ReflectionClass($className);
        if (!$reflection->implementsInterface('Varien_Log_FactoryInterface')) {
            #require_once 'Varien/Log/Exception.php';
            throw new Varien_Log_Exception(
                $className . ' does not implement Varien_Log_FactoryInterface and can not be constructed from config.'
            );
        }
    ...
    protected function getClassName($config, $type, $defaultNamespace)
    {
        if (!isset($config[$type . 'Name'])) {
            #require_once 'Varien/Log/Exception.php';
            throw new Varien_Log_Exception("Specify {$type}Name in the configuration array");
        }
    ...
    {
        /** @var Varien_Log_Writer_Abstract $writer */
     ...
     *     instead of
     *   $log->log('message', Varien_Log::PRIORITY_NAME)
     *
        ...
        if (($priority = array_search($priority, $this->_priorities)) !== false) {
            switch (count($params)) {
                case 0:
                    /** @see Varien_Log_Exception */
                    #require_once 'Varien/Log/Exception.php';
                    throw new Varien_Log_Exception('Missing log message');
                case 1:
            ...
            /** @see Varien_Log_Exception */
            #require_once 'Varien/Log/Exception.php';
            throw new Varien_Log_Exception('Bad log priority');
        }
    }
    ...
    public function log($message, $priority, $extras = null)
    {
        ...
        if (empty($this->_writers)) {
            /** @see Varien_Log_Exception */
            #require_once 'Varien/Log/Exception.php';
            throw new Varien_Log_Exception('No writers were added');
        }
    ...
    public function addPriority($name, $priority)
    {
        ...
        if (isset($this->_priorities[$priority])
            ...
            /** @see Varien_Log_Exception */
            #require_once 'Varien/Log/Exception.php';
            throw new Varien_Log_Exception('Existing priorities cannot be overwritten');
        }
     ...
     *
     * @param  int|Varien_Config|array|Varien_Log_Filter_Interface $filter
     * @return $this
     * @throws Varien_Log_Exception
     */
    ...
    {
        if (is_int($filter)) {
            /** @see Varien_Log_Filter_Priority */
            #require_once 'Varien/Log/Filter/Priority.php';
            $filter = new Varien_Log_Filter_Priority($filter);

        } elseif ($filter instanceof Varien_Config || is_array($filter)) {
            $filter = $this->_constructFilterFromConfig($filter);

        } elseif(! $filter instanceof Varien_Log_Filter_Interface) {
            /** @see Varien_Log_Exception */
            #require_once 'Varien/Log/Exception.php';
     ...
     *
     * @param  mixed $writer Varien_Log_Writer_Abstract or Config array
     * @return Varien_Log
     * @throws Varien_Log_Exception
     */
    public function addWriter($writer)
    {
        if (is_array($writer) || $writer instanceof  Varien_Config) {
        ...
        if (!$writer instanceof Varien_Log_Writer_Abstract) {
            ...
            #require_once 'Varien/Log/Exception.php';
            throw new Varien_Log_Exception(
                'Writer must be an instance of Varien_Log_Writer_Abstract'
    ...
    public function registerErrorHandler()
    {
        ...
        // Contruct a default map of phpErrors to Varien_Log priorities.
        // Some of the errors are uncatchable, but are included for completeness
        $this->_errorHandlerMap = array(
            E_NOTICE            => Varien_Log::NOTICE,
            E_USER_NOTICE       => Varien_Log::NOTICE,
            E_WARNING           => Varien_Log::WARN,
            E_CORE_WARNING      => Varien_Log::WARN,
            E_USER_WARNING      => Varien_Log::WARN,
            E_ERROR             => Varien_Log::ERR,
            E_USER_ERROR        => Varien_Log::ERR,
            E_CORE_ERROR        => Varien_Log::ERR,
            E_RECOVERABLE_ERROR => Varien_Log::ERR,
            E_STRICT            => Varien_Log::DEBUG,
        );
        // PHP 5.3.0+
        if (defined('E_DEPRECATED')) {
            $this->_errorHandlerMap['E_DEPRECATED'] = Varien_Log::DEBUG;
        }
        if (defined('E_USER_DEPRECATED')) {
            $this->_errorHandlerMap['E_USER_DEPRECATED'] = Varien_Log::DEBUG;
        }
