<?php
class Varien_Cache_Backend_File extends Varien_Cache_Backend implements Varien_Cache_Backend_ExtendedInterface
{
    /**
     * Available options
     *
    ...
    public function __construct(array $options = array())
    {
    	...
    	/***
    	 * array(4) { 
    	 * 	["hashed_directory_level"]=> int(1) 
    	 * 	["hashed_directory_perm"]=> int(511) 
    	 * 	["file_name_prefix"]=> string(4) "mage" 
    	 *  ["cache_dir"]=> string(30) "D:\xampp\htdocs\mage\var\cache" 
    	 * }
    	 */
        ...
        if (isset($this->_options['file_name_prefix'])) { // particular case for this option
            if (!preg_match('~^[a-zA-Z0-9_]+$~D', $this->_options['file_name_prefix'])) {
                Varien_Cache::throwException('Invalid file_name_prefix : must use only [a-zA-Z0-9_]');
            }
        }
        if ($this->_options['metadatas_array_max_size'] < 10) {
            Varien_Cache::throwException('Invalid metadatas_array_max_size, must be > 10');
        }
    ...
    {
        if (!is_dir($value)) {
            Varien_Cache::throwException(sprintf('cache_dir "%s" must be a directory', $value));
        }
        if (!is_writable($value)) {
            Varien_Cache::throwException(sprintf('cache_dir "%s" is not writable', $value));
        }
    ...
    public function load($id, $doNotTestCacheValidity = false)
    {
        ...
        if ($this->_options['read_control']) {
            ...
            if ($hashData != $hashControl) {
                ...
                $this->_log('Varien_Cache_Backend_File::load() / read_control : stored hash and computed hash do not match');
                $this->remove($id);
                return false;
            }
        }
    ...
    public function save($data, $id, $tags = array(), $specificLifetime = false)
    {
        ...
        if (!$res) {
            $this->_log('Varien_Cache_Backend_File::save() / error on saving metadata');
            return false;
        }
     ...
     * Available modes are :
     *
     * Varien_Cache::CLEANING_MODE_ALL (default)    => remove all cache entries ($tags is not used)
     * Varien_Cache::CLEANING_MODE_OLD              => remove too old cache entries ($tags is not used)
     * Varien_Cache::CLEANING_MODE_MATCHING_TAG     => remove cache entries matching all given tags
     *                                               ($tags can be an array of strings or a single string)
     * Varien_Cache::CLEANING_MODE_NOT_MATCHING_TAG => remove cache entries not {matching one of the given tags}
     *                                               ($tags can be an array of strings or a single string)
     * Varien_Cache::CLEANING_MODE_MATCHING_ANY_TAG => remove cache entries matching any given tags
     *                                               ($tags can be an array of strings or a single string)
     *
     ...
     * @return boolean true if no problem
     */
    public function clean($mode = Varien_Cache::CLEANING_MODE_ALL, $tags = array())
    {
    ...
    public function getFillingPercentage()
    {
        ...
        if ($total == 0) {
            Varien_Cache::throwException('can\'t get disk_total_space');
        } else {
            if ($free >= $total) {
                return 100;
            }
    ...
    protected function _remove($file)
    {
        ...
        if (!@unlink($file)) {
            ...
            $this->_log("Varien_Cache_Backend_File::_remove() : we can't remove $file");
            return false;
        }
        return true;
    }
     ...
     * @return boolean True if no problem
     */
    protected function _clean($dir, $mode = Varien_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        if (!is_dir($dir)) {
            return false;
        }
        ...
        foreach ($glob as $file)  {
            if (is_file($file)) {
                ...
                if ($this->_isMetadatasFile($fileName)) {
                    ...
                    if ($mode == Varien_Cache::CLEANING_MODE_ALL) {
                        $metadataFiles[] = $file;
                    }
                    continue;
                }
                ...
                switch ($mode) {
                    case Varien_Cache::CLEANING_MODE_ALL:
                        $result = $result && $this->remove($id);
                        break;
                    case Varien_Cache::CLEANING_MODE_OLD:
                        if (time() > $metadatas['expire']) {
                            $result = $this->remove($id) && $result;
                        }
                        break;
                    case Varien_Cache::CLEANING_MODE_MATCHING_TAG:
                        $matching = true;
                        foreach ($tags as $tag) {
                        ...
                        break;
                    case Varien_Cache::CLEANING_MODE_NOT_MATCHING_TAG:
                        $matching = false;
                        foreach ($tags as $tag) {
                            if (in_array($tag, $metadatas['tags'])) {
                                $matching = true;
                        ...
                        break;
                    case Varien_Cache::CLEANING_MODE_MATCHING_ANY_TAG:
                        $matching = false;
                        foreach ($tags as $tag) {
                            if (in_array($tag, $metadatas['tags'])) {
                                $matching = true;
                        ...
                        break;
                    default:
                        Varien_Cache::throwException('Invalid mode for clean() method');
                        break;
                }
            }
            ...
            if (is_file($file)) {
                ...
                switch ($mode) {
                    ...
                    case 'matchingAny':
                        ...
                        break;
                    default:
                        Varien_Cache::throwException('Invalid mode for _get() method');
                        break;
                }
            }
            if ((is_dir($file)) and ($this->_options['hashed_directory_level']>0)) {
                ...
                if ($recursiveRs === false) {
                    $this->_log('Varien_Cache_Backend_File::_get() / recursive call : can\'t list entries of "'.$file.'"');
                } else {
    ...
    protected function _hash($data, $controlType)
    {
        ...
        default:
            Varien_Cache::throwException("Incorrect hash function : $controlType");
        }
    }
