...
class Magento_Db_Select
{
     ...
     * Class constructor
     *
     * @param Varien_Db_Adapter_Abstract $adapter
     */
    public function __construct(Varien_Db_Adapter_Abstract $adapter)
    {
     ...
     * @return Magento_Db_Select This Magento_Db_Select object.
     */
     ...
     * table.
     *
     * The second parameter can be a single string or Magento_Db_Expr object,
     * or else an array of strings or Magento_Db_Expr objects.
     *
     * The first parameter can be null or an empty string, in which case
     * no correlation name is generated or prepended to the columns named
     * in the second parameter.
     *
     * @param  array|string|Magento_Db_Expr $name The table name or an associative array
     *                                         relating correlation name to table name.
     * @param  array|string|Magento_Db_Expr $cols The columns to select from this table.
     ...
     *
     * The parameter can be a single string or Magento_Db_Expr object,
     * or else an array of strings or Magento_Db_Expr objects.
     *
     * @param  array|string|Magento_Db_Expr $cols The columns to select from this table.
    ...
    public function columns($cols = '*', $correlationName = null)
    {
        ...
        if (!array_key_exists($correlationName, $this->_parts[self::FROM])) {
            /**
             * @see Varien_Db_Exception
             */
            #require_once 'Zend/Db/Select/Exception.php';
            throw new Varien_Db_Exception("No table has been specified for the FROM clause");
        }
     ...
     *
     * The first parameter has to be an array of Magento_Db_Select or
     * sql query strings.
     *
     * <code>
     * $sql1 = $db->select();
    ...
    public function union($select = array(), $type = self::SQL_UNION)
    {
        if (!is_array($select)) {
            #require_once 'Zend/Db/Select/Exception.php';
            throw new Varien_Db_Exception(
                "union() only accepts an array of Magento_Db_Select instances of sql query strings."
            );
        }

        if (!in_array($type, self::$_unionTypes)) {
            #require_once 'Zend/Db/Select/Exception.php';
            throw new Varien_Db_Exception("Invalid union type '{$type}'");
        }
     ...
     *
     * @param  array|string|Magento_Db_Expr $name The table name.
    ...
    public function getPart($part)
    {
        ...
        if (!array_key_exists($part, $this->_parts)) {
            throw new Varien_Db_Exception("Invalid Select part '$part'");
        }
     ...
     * particular Magento_Db_Select object.
     *
     ...
     * @param  array|string|Magento_Db_Expr $name Table name
     * @param  string $cond Join on this condition
     * @param  array|string $cols The columns to select from the joined table
     * @param  string $schema The database name to specify, if any.
     * @return Magento_Db_Select This Magento_Db_Select object
    ...
    protected function _join($type, $name, $cond, $cols, $schema = null)
    {
        if (!in_array($type, self::$_joinTypes) && $type != self::FROM) {
            throw new Varien_Db_Exception("Invalid join type '$type'");
        }

        if (count($this->_parts[self::UNION])) {
            throw new Varien_Db_Exception("Invalid use of table with " . self::SQL_UNION);
        }
        ...
        } elseif ($name instanceof Magento_Db_Expr|| $name instanceof Magento_Db_Select) {
            $tableName = $name;
        ...
        if (!empty($correlationName)) {
            if (array_key_exists($correlationName, $this->_parts[self::FROM])) {
                throw new Varien_Db_Exception("You cannot define a correlation name '$correlationName' more than once");
            }
    ...
    public function _joinUsing($type, $name, $cond, $cols = '*', $schema = null)
    {
        if (empty($this->_parts[self::FROM])) {
            #require_once 'Zend/Db/Select/Exception.php';
            throw new Varien_Db_Exception("You can only perform a joinUsing after specifying a FROM table");
        }
    ...
    protected function _tableCols($correlationName, $cols, $afterCorrelationName = null)
    {
        ...
        foreach (array_filter($cols) as $alias => $col) {
            ...
            if (is_string($col)) {
                ...
                // Check for columns that look like functions and convert to Magento_Db_Expr
                if (preg_match(self::REGEX_COLUMN_EXPR, (string) $col)) {
                    $col = new Magento_Db_Expr($col);
    ...
    protected function _where($condition, $value = null, $type = null, $bool = true)
    {
        if (count($this->_parts[self::UNION])) {
            #require_once 'Zend/Db/Select/Exception.php';
            throw new Varien_Db_Exception("Invalid use of where clause with " . self::SQL_UNION);
        }
    ...
    protected function _renderColumns($sql)
    {
        ...
        foreach ($this->_parts[self::COLUMNS] as $columnEntry) {
            ...
            if ($column instanceof Magento_Db_Expr) {
                $columns[] = $this->_adapter->quoteColumnAs($column, $alias, true);
            } else {
                if ($column == self::SQL_WILDCARD) {
                    $column = new Magento_Db_Expr(self::SQL_WILDCARD);
                    $alias = null;
                }
                if (empty($correlationName)) {
                    $columns[] = $this->_adapter->quoteColumnAs($column, $alias, true);
    ...
    protected function _renderUnion($sql)
    {
        if ($this->_parts[self::UNION]) {
            ...
            foreach ($this->_parts[self::UNION] as $cnt => $union) {
                ...
                if ($target instanceof Magento_Db_Select) {
     ...
     * @return Magento_Db_Select
     * @throws Varien_Db_Exception If an invalid method is called.
     */
        ...
        if (preg_match('/^join([a-zA-Z]*?)Using$/', $method, $matches)) {
            ...
            if ($type) {
                ...
                if (!in_array($type, self::$_joinTypes)) {
                    #require_once 'Zend/Db/Select/Exception.php';
                    throw new Varien_Db_Exception("Unrecognized method '$method()'");
                }
                if (in_array($type, array(self::CROSS_JOIN, self::NATURAL_JOIN))) {
                    #require_once 'Zend/Db/Select/Exception.php';
                    throw new Varien_Db_Exception("Cannot perform a joinUsing with method '$method()'");
                }
            } else {
