<?php
class Magento_Cache_Core
{
    /**
     * Messages
     */
    const BACKEND_NOT_SUPPORTS_TAG = 'tags are not supported by the current backend';
    const BACKEND_NOT_IMPLEMENTS_EXTENDED_IF = 'Current backend doesn\'t implement the Varien_Cache_Backend_ExtendedInterface, so this method is not available';

    /**
     * Backend Object
     *
     * @var Varien_Cache_Backend_Interface $_backend
     */
    ...
    /**
     * True if the backend implements Varien_Cache_Backend_ExtendedInterface
     *
    ...
    /**
     * Array of capabilities of the backend (only if it implements Varien_Cache_Backend_ExtendedInterface)
     *
     * @var array
     */
     ...
     * Constructor
     *
     * @param  array|Varien_Config $options Associative array of options or Varien_Config instance
    ...
    public function __construct($options = array())
    {
        ...
        if (!is_array($options)) {
            throw new Varien_Cache_Exception("Options passed were not an array"
            . " or Varien_Config instance.");
        }
    ...
    /**
     * Set options using an instance of type Varien_Config
     *
     * @param Varien_Config $config
     * @return Zend_Cache_Core
     */
    public function setConfig(Varien_Config $config)
    {
     ...
     * Set the backend
     *
     * @param  Varien_Cache_Backend $backendObject
     * @throws Zend_Cache_Exception
     * @return void
     */
    public function setBackend(Varien_Cache_Backend $backendObject)
    {
        $this->_backend= $backendObject;
        // some options (listed in $_directivesList) have to be given
        // to the backend too (even if they are not "backend specific")
        $directives = array();
        foreach (Magento_Cache_Core::$_directivesList as $directive) {
            $directives[$directive] = $this->_options[$directive];
        }
        $this->_backend->setDirectives($directives);
        if (in_array('Varien_Cache_Backend_ExtendedInterface', class_implements($this->_backend))) {
     ...
     * Returns the backend
     *
     * @return Varien_Cache_Backend backend object
     */
    ...
    {
        if (!is_string($name)) {
            throw new Varien_Cache_Exception("Incorrect option name!");
        }
    ...
    public function getOption($name)
    {
        ...
        throw new Varien_Cache_Exception("Incorrect option name : $name");
    }
    ...
    public function save($data, $id = null, $tags = array(), $specificLifetime = false, $priority = 8)
    {
        ...
        if ($this->_options['automatic_serialization']) {
            ...
            if (!is_string($data)) {
                throw new Varien_Cache_Exception("Datas must be string or set automatic_serialization = true");
            }
        }
    ...
    public function clean($mode = 'all', $tags = array())
    {
        ...
        if (!in_array($mode, array(Zend_Cache::CLEANING_MODE_ALL,
            ...
            throw new Varien_Cache_Exception('Invalid cleaning mode');
        }
        $this->_validateTagsArray($tags);
    ...
    public function getIdsMatchingTags($tags = array())
    {
        if (!$this->_extendedBackend) {
            throw new Varien_Cache_Exception(self::BACKEND_NOT_IMPLEMENTS_EXTENDED_IF);
        }
        if (!($this->_backendCapabilities['tags'])) {
            throw new Varien_Cache_Exception(self::BACKEND_NOT_SUPPORTS_TAG);
        }
    ...
    {
        if (!is_string($string)) {
            throw new Varien_Cache_Exception('Invalid id or tag : must be a string');
        }
        if (substr($string, 0, 9) == 'internal-') {
            throw new Varien_Cache_Exception('"internal-*" ids or tags are reserved');
        }
        if (!preg_match('~^[a-zA-Z0-9_]+$~D', $string)) {
            throw new Varien_Cache_Exception("Invalid id or tag '$string' : must use only [a-zA-Z0-9_]");
        }
    }
    ...
    {
        if (!is_array($tags)) {
            throw new Varien_Cache_Exception('Invalid tags array : must be an array');
        }
        foreach($tags as $tag) {
    ...
    protected function _log($message, $priority = 4)
    {
        ...
        if (!(isset($this->_options['logger']) || $this->_options['logger'] instanceof Zend_Log)) {
            throw new Varien_Cache_Exception('Logging is enabled but logger is not set');
        }
