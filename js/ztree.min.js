...
(function (p) {
    var G, H, I, J, K, L, r = {}, u = {}, v = {}, M = {
        ...
        data: {
            key: {children: "children", name: "name", title: "", url: "url",type:"type"},
        ...
        a.bind(c.SELECTED,
            function (a, c, g, l) {
                j.apply(b.callback.onSelected, [c, g, l])
            });
        a.bind(c.UNSELECTED, function (a, c, g, l) {
            j.apply(b.callback.onUnSelected, [c, g, l])
        })
    }], x = [function (b) {
        var a = e.event;
        ...
        var a = b.target, c = h.getSetting(b.data.treeId), d = "", f = null, g = "", l = "", i = null, o = null, k = null;
        if (j.eqs(b.type, "mousedown"))l = "mousedown"; else if (j.eqs(b.type, "mouseup"))l = "mouseup"; else if (j.eqs(b.type, "contextmenu"))l = "contextmenu"; else if (j.eqs(b.type, "click"))if (j.eqs(a.tagName, "span") && a.getAttribute("treeNode" + e.id.SWITCH) !== null)d = j.getNodeMainDom(a).id, g = "switchNode"; else {
            if (k = j.getMDom(c, a, [{tagName: "a", attrName: "treeNode" + e.id.A}]))d = j.getNodeMainDom(k).id,
                g = "clickNode"
        ...
        return {stop: !1, node: f, nodeEventType: g, nodeEventCallback: i, treeEventType: l, treeEventCallback: o}
    }], A = [function (b) {
        var a = h.getRoot(b);
        a || (a = {}, h.setRoot(b, a));
        ...
        addNodeCache: function (b, a) {
            h.getCache(b).nodes[h.getNodeCacheId(a.tId)] =
                a
            ...
            b.data.simpleData.enable && (a[b.data.simpleData.pIdKey] =
                a.parentTId ? a.getParentNode()[b.data.simpleData.idKey] : b.data.simpleData.rootPId)
            ...
            return "" +
                a[b.data.key.type]
        },getAllChildrenNodes:function(b,a){
            ...
            if (!a || !c)return [];
            for (var f = b.data.key.children, g = [], d = d.toLowerCase(), l = 0, e = a.length; l < e; l++)typeof a[l][c] ==
            "string" && a[l][c].toLowerCase().indexOf(d) > -1 && g.push(a[l]), g = g.concat(h.getNodesByParamFuzzy(b, a[l][f], c, d));
            return g
        }, getNodesByFilter: function (b, a, c, d, f) {
            if (!a)return d ? null : [];
            for (var g = b.data.key.children, e = d ? null : [], i = 0, o = a.length; i < o; i++) {
            ...
            if (!a)return null;
            for (var c = b.data.key.children, d = a.parentTId ? a.getParentNode() : h.getRoot(b),
                     f = 0, g = d[c].length; f < g; f++)if (d[c][f] === a)return f == 0 ? null : d[c][f - 1];
            return null
        }, getRoot: function (b) {
            ...
            for (var a =
                0, c = A.length; a < c; a++)A[a].apply(this, arguments)
            ...
            for (var c = h.getRoot(b), d = 0, f = c.curSelectedList.length; d < f; d++)if (a === c.curSelectedList[d] || !h.getNodeCache(b,
                    c.curSelectedList[d].tId))c.curSelectedList.splice(d, 1), d--, f--
        ...
        bindEvent: function (b) {
            ...
            for (var a = 0, c = x.length; a <
            c; a++)x[a].apply(this, arguments)
        }, bindTree: function (b) {
            ...
            b.treeObj.unbind("click",
                m.proxy).unbind("dblclick", m.proxy).unbind("mouseover", m.proxy).unbind("mouseout", m.proxy).unbind("mousedown", m.proxy).unbind("mouseup", m.proxy).unbind("contextmenu", m.proxy)
        }, doProxy: function (b) {
            ...
            for (var a = m.doProxy(b), c = !0, d = 0, f = a.length; d < f; d++) {
                var g = a[d];
                g.nodeEventCallback && (c = g.nodeEventCallback.apply(g, [b, g.node]) &&
                    c);
    ...
    H = function (b, a) {
        var c = h.getSetting(b.data.treeId), d = c.view.autoCancelSelected && (b.ctrlKey || b.metaKey) && h.isSelectedNode(c, a) ? 0 : c.view.autoCancelSelected && (b.ctrlKey ||
        b.metaKey) && c.view.selectedMulti ? 2 : 1;
    ...
    var j = {
        apply: function (b, a, c) {
            ...
            return typeof b == "string" ? p(b, c ? c.treeObj.get(0).ownerDocument :
                null) : p("#" + b.tId + a, c ? c.treeObj : null)
        ...
        addNodes: function (b, a, c, d) {
            ...
            if (!c)return [];
            for (var e =
                [], j = b.data.key.children, k = 0, m = c.length; k < m; k++) {
                var n = c[k];
                if (f) {
                    var t = (d ? d : h.getRoot(b))[j].length == c.length && k == 0;
                    h.initNode(b, a, n, d, t, k == c.length - 1, g);
                    h.addNodeCache(b, n)
                }
                t = [];
                n[j] && n[j].length > 0 && (t = i.appendNodes(b, a + 1, n[j], n, f, g && n.open));
                g && (i.makeDOMNodeMainBefore(e, b, n), i.makeDOMNodeLine(e, b, n), h.getBeforeA(b, n, e), i.makeDOMNodeNameBefore(e, b, n), h.getInnerBeforeA(b, n, e), i.makeDOMNodeIcon(e, b, n), h.getInnerAfterA(b, n, e), i.makeDOMNodeNameAfter(e, b, n), h.getAfterA(b, n, e), n.isParent && n.open && i.makeUlHtml(b,
                    n, e, t.join("")), i.makeDOMNodeMainAfter(e, b, n), h.addCreatedNode(b, n))
            }
            return e
            ...
            for (f = 0, g = b.async.autoParam.length; a && f < g; f++) {
                var q = b.async.autoParam[f].split("="), o = q;
            ...
            p.ajax({
                ...
                success: function (f) {
                    if (n == h.getRoot(b)._ver) {
                        ...
                        b.treeObj.trigger(e.event.ASYNC_SUCCESS, [b.treeId,
                            a, f]);
                        j.apply(d)
                    }
                },
            ...
            var d = h.getRoot(b).curSelectedList, f, g;
            for (f = d.length - 1; f >= 0; f--)if (g = d[f], a === g || !a && (!c || c !== g))if (k(g, e.id.A, b).removeClass(e.node.CURSELECTED), a) {
                h.removeSelectedNode(b, a);
                b.treeObj.trigger(e.event.UNSELECTED, [m, b.treeId, g]);
                break
            } else d.splice(f, 1), b.treeObj.trigger(e.event.UNSELECTED,
                [m, b.treeId, g])
            ...
            if (c && c.length != 0) {
                ...
                d ? (d = k(d, e.id.UL, b), d.get(0) && d.append(a.join(""))) :
                    b.treeObj.append(a.join(""));
            ...
            if (a) {
                ...
                if (!a.open && a.isParent && (!k(a, e.id.UL, b).get(0) || a[l] && a[l].length >
                    0 && !k(a[l][0], b).get(0)))i.appendParentULDom(b, a), i.createNodeCallback(b);
                if (a.open == c)j.apply(f, []); else {
                    var c = k(a, e.id.UL, b), g = k(a, e.id.SWITCH, b), o = k(a, e.id.ICON, b);
                    a.isParent ? (a.open = !a.open, a.iconOpen && a.iconClose && o.attr("style", i.makeNodeIcoStyle(b, a)), a.open ? (i.replaceSwitchClass(a, g, e.folder.OPEN), i.replaceIcoClass(a, o, e.folder.OPEN), d == !1 || b.view.expandSpeed == "" ? (c.show(), j.apply(f, [])) : a[l] && a[l].length > 0 ? c.slideDown(b.view.expandSpeed, f) : (c.show(), j.apply(f, []))) : (i.replaceSwitchClass(a,
                        g, e.folder.CLOSE), i.replaceIcoClass(a, o, e.folder.CLOSE), d == !1 || b.view.expandSpeed == "" || !(a[l] && a[l].length > 0) ? (c.hide(), j.apply(f, [])) : c.slideUp(b.view.expandSpeed, f))) : j.apply(f, [])
                }
            } else j.apply(f, [])
            ...
            var g = h.getRoot(b), e = b.data.key.children, g = a ? a[e] : g[e], e = a ? !1 :
            ...
            var t = h.getNodeType(a,c),  t = a.view.nameIsHTML ? t : t.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
            if(t == 3){
                var nameStyle = 'margin-left:6px;width:22px;height:22px;'
            }
            b.push("<span id='",
                c.tId, e.id.ICON, "' title='' treeNode", e.id.ICON, " class='", i.makeNodeIcoClass(a, c), "' style='", i.makeNodeIcoStyle(a, c), "'></span><span style='",nameStyle,"' id='", c.tId, e.id.SPAN, "'>", d, "</span>")
        }, makeDOMNodeLine: function (b, a, c) {
            var style = ''
            if($('.chose-tree-wrap').length > 0){//选择人和部门的改写
                var left = (-18)*c.level+'px';
                var pleft= 18 * c.level+'px';
                style = "left:"+left+";padding-left:"+pleft;
                //console.log(style)
            }
            // console.log(c.type)
            var t = h.getNodeType(a, c), t = a.view.nameIsHTML ? t : t.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
            b.push("<div type='", c.type,"' id='",c.tId, e.id.WRAP,"' level='",c.level,"' style='",style,"'><span id='", c.tId, e.id.SWITCH, "' title='' class='", i.makeNodeLineClass(a, c), "' treeNode", e.id.SWITCH, "></span>")
        ...
        makeDOMNodeNameAfter: function (b) {
            ...
            j.apply(a.view.showTitle, [a.treeId, c], a.view.showTitle) && d && b.push("title='", d.replace(/'/g,
                "&#39;").replace(/</g, "&lt;").replace(/>/g, "&gt;"), "'");
            b.push(">")
            ...
            var c = [];
            if (!a.isAjaxing) {
                var d = a.isParent && a.iconOpen &&
                a.iconClose ? a.open ? a.iconOpen : a.iconClose : a.icon;
            ...
            a.isParent ? c.push(a.open ? e.folder.OPEN : e.folder.CLOSE) :
        ...
        makeUlLineClass: function (b, a) {
            ...
            if (h.getNodeCache(b, a.tId)) {
                ...
                i.setLastNode(b, f);
                var j, g = f[d].length;
                if (!b.data.keep.parent && g == 0)f.isParent = !1, f.open = !1, g = k(f, e.id.UL, b), l = k(f, e.id.SWITCH, b), j = k(f, e.id.ICON, b), i.replaceSwitchClass(f, l, e.folder.DOCU), i.replaceIcoClass(f, j, e.folder.DOCU), g.css("display", "none"); else if (b.view.showLine && g > 0) {
                    var o = f[d][g - 1], g = k(o, e.id.UL, b), l = k(o, e.id.SWITCH, b);
                    j = k(o, e.id.ICON, b);
                    f == c ? f[d].length == 1 ? i.replaceSwitchClass(o, l, e.line.ROOT) : (c = k(f[d][0], e.id.SWITCH, b), i.replaceSwitchClass(f[d][0], c, e.line.ROOTS),
                        i.replaceSwitchClass(o, l, e.line.BOTTOM)) : i.replaceSwitchClass(o, l, e.line.BOTTOM);
            ...
            b.treeObj.trigger(e.event.SELECTED, [m, b.treeId, a])
            ...
            if (a) {
                var c =
                    k(a, e.id.SWITCH, b), d = k(a, e.id.UL, b), f = k(a, e.id.ICON, b), g = i.makeUlLineClass(b, a);
            ...
            j.apply(b.view.showTitle,
                [b.treeId, a], b.view.showTitle) && k(a, e.id.A, b).attr("title", !c ? "" : c)
    ...
    p.fn.zTree = {
        consts: {
            className: {
                BUTTON: "button",
                LEVEL: "level", ICO_LOADING: "ico_loading", SWITCH: "switch"
            },
            event: {
            ...
            id: {A: "_a", ICON: "_ico", SPAN: "_span", SWITCH: "_switch", UL: "_ul",WRAP:"_wrap"},
            ...
            c = {
                setting: d, addNodes: function (a, b, c) {
                    ...
                    return !a ?
                        null : h.getNodesByParamFuzzy(d, c ? c[d.data.key.children] : h.getNodes(d), a, b)
                    ...
                    if (a && j.uCanDo(d)) {
                        ...
                        if (a.parentTId)i.expandCollapseParentNode(d, a.getParentNode(),
                            !0, !1, function () {
                                try {
                                    k(a, d).focus().blur()
                                } catch (b) {
            ...
            b.treeTools = c;
            h.setZTreeTools(d, c);
            b[a] && b[a].length > 0 ? i.createNodes(d, 0, b[a]) : d.async.enable &&
...
(function (m) {
    ...
    f.addBeforeA(function (c, a, b) {
        c.check.enable && (f.makeChkFlag(c, a), b.push("<span ID='", a.tId, j.id.CHECK, "' class='", e.makeChkClass(c, a), "' treeNode", j.id.CHECK, a.nocheck === !0 ? " style='display:none;'" : "", " _checked='",false,"'></span>"))
    });
                ...
                addRemoveBtn: function (b, a) {
                    ...
                    if (a != c && (!b.data.keep.leaf || !a || a.isParent || r != d.move.TYPE_INNER)) {
                        ...
                        !b.data.keep.parent && g[e].length < 1 ? (g.isParent = !1,
                            g.open = !1, a = o(g, d.id.UL, b), r = o(g, d.id.SWITCH, b), e = o(g, d.id.ICON, b), f.replaceSwitchClass(g, r, d.folder.DOCU), f.replaceIcoClass(g, e, d.folder.DOCU), a.css("display", "none")) : x && f.setNodeLineIcos(b, x);
