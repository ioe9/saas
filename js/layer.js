...
Class.pt.config = {
    ...
    success: function(layer,index){}, //创建成功后的回调
...
Class.pt.space = function(html){
    ...
    frame = [
    '<div class="xubox_dialog">'+ ico +'<span class="xubox_msg xubox_text" style="'+ (ico ? '' : 'padding-left:10px') +'">' + dialog.msg + '</span></div>',
...
Class.pt.set = function(times){
    ...
    switch(config.type){
        case 0:
            ...
            if(config.title){
                layerE.find(doms[3]).css({paddingTop: 15 + layerTitle.outerHeight()});
            }else{
...
Class.pt.callback = function(){
    ...
    layer.ie6 && that.IE6(layerE,that.index);
