<?php
class Mage_Edm_Helper_Template extends Mage_Core_Helper_Abstract {
	...
	protected function _initPreview() {
		...
		foreach ($collection as $_c) {
			$arr[trim($_c->getModuleName())] = array (
				'info' => $_c
			);
		}
		$this->_modules = $arr;
	 ...
	 * return String
	 */
	public function render($content, $moduleName = false, $moduleConfig = false, $templateId = false, $itemNum = 1) {
		$this->_initPreview();
	...
	public function processModule($value, $decor = true, $moduleName = false, $moduleConfig = true, $templateId = false, $itemNum = 1) {
		...
		if (preg_match_all(self :: PATTERN_MODULE, $value, $constructions, PREG_SET_ORDER)) {
			...
			foreach ($constructions as $index => $construction) {
				...
				if (!$moduleName) {
					//Mage::log($construction[0],null,'t.log');
					$i++;
					$uid = uniqid() . $i;
					$tempDecor = "<div class='module_handle' data-name='" . str_replace(array ('[',']'), '', $construction[0]) . "' id='module_handle_" . $uid . "'>" . $construction[0] . '</div>';
					$value = str_replace($construction[0], $tempDecor, $value);
					$decorDivs[$construction[0]] = $tempDecor;
		...
		if (preg_match_all(self :: PATTERN_MODULE, $value, $constructions, PREG_SET_ORDER)) {
			...
			foreach ($constructions as $index => $construction) {
				...
				if ($construction[0] == "[[公司优势]]") {
					...
					$advRel1 = Mage :: getResourceModel('edm/company_advantage_attrvalue_collection')->addFieldToFilter('attr_id', 2)->addFieldToFilter('advantage_id', array (
						'in' => $advIds
					))->addFieldToFilter('value', $clientTouchPoints[0]);
					foreach ($advRel1 as $_v) {
						array_push($selectedAdsIds1, $_v->getData('advantage_id'));
					}
					$advRel2 = Mage :: getResourceModel('edm/company_advantage_attrvalue_collection')->addFieldToFilter('attr_id', 2)->addFieldToFilter('advantage_id', array (
						'in' => $advIds
					...
					$tmpAdvRel1 = array ();
					$tmpAdvRel2 = array ();
					$advRel1 = Mage :: getResourceModel('edm/company_advantage_value_collection')->addFieldToFilter('advantage_id', array (
						'in' => $selectedAdsIds1
					))->addFieldToFilter('company_id', $company->getId());

					$advRel2 = Mage :: getResourceModel('edm/company_advantage_value_collection')->addFieldToFilter('advantage_id', array (
						'in' => $selectedAdsIds2
					))->addFieldToFilter('company_id', $company->getId());

					foreach ($advRel1 as $_t) {
						if (!array_key_exists($_t->getData('advantage_id'), $tmpAdvRel1)) {
							$tmpAdvRel1[$_t->getData('advantage_id')] = array (
								$_t
							);
						} else {
					...
					foreach ($advRel2 as $_t) {
						if (!array_key_exists($_t->getData('advantage_id'), $tmpAdvRel2)) {
							$tmpAdvRel2[$_t->getData('advantage_id')] = array (
								$_t
							);
						} else {
				...
				if ($construction[0] == "[[称呼]]") {
					...
					$moduleChenghu = 2;
					$tmp = array ();
					$items = Mage :: getResourceModel('edm/templates_module_item_collection')->addFieldToFilter('status', 1)->addFieldToFilter('module_id', $moduleChenghu);
							...
							if (count($items)) {
								foreach ($items as $_item) {
									if ($itemParent = $_item->getData('item_parent')) {
										$relates = Mage :: getResourceModel('edm/templates_module_item_relate_collection')->addFieldToFilter('item_id', $itemParent);
									} else {
									...
									foreach ($relates as $_r) {
										...
										if ($_r->getData('type') == 'client_attr') { //客户信息，判断当前客户是否有该属性
											...
											if ($_r->getData('type') == 'company_attr') { //会员信息，判断当前会员是否有该属性
												...
												if ($_r->getData('type') == 'company_advantage') { //公司优势，判断当前公司是否有该优势
													//echo "<xmp>";var_dump($this->_previewCompany[$name]);echo "</xmp>";
													...
													if ($_r->getData('type') == 'contact_attr') { //联系人信息，判断当前联系人是否有该属性
									...
									//这里处理项-客户属性关联关系
									if ($itemParent) {
										$attrRelates = Mage :: getResourceModel('edm/templates_module_item_attrvalue_collection')->addFieldToFilter('item_id', $itemParent);
									} else {
										$attrRelates = Mage :: getResourceModel('edm/templates_module_item_attrvalue_collection')->addFieldToFilter('item_id', $_item->getId());
									}

									$attrRelates->getSelect()->join(array (
										'attr' => 'edm_client_attr'
									), 'attr.attr_id=main_table.attr_id', array (
										'name'
									));
									...
									foreach ($attrRelates as $_r) {
										if ($_r->getValue) {
											if (!array_key_exists($_r->getName())) {
												$attrRelateValues[$_r->getName()] = array (
													$_r->getValue
												);
											} else {
		...
		if (preg_match_all(self :: PATTERN_MODULE, $value, $constructions, PREG_SET_ORDER)) {
			...
			foreach ($constructions as $index => $construction) {
				...
				$value = str_replace($decorDivs[$construction[0]], '', $value);
			}
		}
		return $value;
	...
	public function getPreviewCompany() {
		...
		$values = Mage :: getResourceModel('edm/company_attr_value_collection')->addFieldToFilter('company_id', $company->getId());
		$values->getSelect()->join(array (
			'attr' => 'edm_company_attr'
		), 'attr.attr_id=main_table.attr_id', array (
			'name'
		));
		...
		/*
		if (!$companyAttrData['产品关键词']) {
			if ($promoProduct) {
				$companyAttrData['产品关键词'] = $promoProduct;
			} else {
				unset($companyAttrData['产品关键词']);
			}
	...
	public function getPreviewCompanyAdvantage() {
		...
		$values = Mage :: getResourceModel('edm/company_advantage_value_collection')->addFieldToFilter('company_id', $company->getId())
		//->addFieldToFilter('enable_scope',0)
	->addFieldToFilter('main_table.value', array (
			'notnull' => true
		));
		$values->getSelect()->join(array (
			'adv' => 'edm_company_advantage'
		), 'adv.advantage_id=main_table.advantage_id', array (
			'code',
			'enable_scope'
		));
		...
		foreach ($values as $_value) {
			if ($_value->getData('enable_scope') == 0) {
				$companyAdvantageData[$_value->getCode()] = array (
					'value' => $_value->getValue(),
					'advantage_id' => $_value->getData('advantage_id')
				);
			} else {

				if (array_key_exists($_value->getCode(), $this->_previewCompanyAdvantageCopy)) {
					$this->_previewCompanyAdvantageCopy[$_value->getCode()] = array (
						array (
							'value' => $_value->getValue(),
							'advantage_id' => $_value->getData('advantage_id')
						)
	...
	public function getPreviewClient() {
		...
		//这里根据字段处理基本属性，根据变量处理自定义属性
		$values = Mage :: getResourceModel('edm/client_attr_value_collection')->addFieldToFilter('client_id', $client->getId());
		$values->getSelect()->join(array (
			'attr' => 'edm_client_attr'
		), 'attr.attr_id=main_table.attr_id', array (
			'name',
		...
		foreach ($values as $_value) {
			if ($_value->getAttrType() == 'multiselect') {
				...
				$_option = Mage :: getModel('edm/client_attr_option')->load($_value->getValue());
				if ($_option->getId()) {
					$_optionValue = $_option->getData('value');
					if (!array_key_exists($_value->getName(), $clientAttrData)) {
						$clientAttrData[$_value->getName()] = array (
							$_optionValue
						);
					} else {
	...
	public function getPreviewContact() {
		...
		//这里根据字段处理基本属性，根据变量处理自定义属性
		$values = Mage :: getResourceModel('edm/client_email_attr_value_collection')->addFieldToFilter('email_id', $contact->getId());
		$values->getSelect()->join(array (
			'attr' => 'edm_client_email_attr'
		), 'attr.attr_id=main_table.attr_id', array (
			'name',
			'attr_type'
		));
		foreach ($values as $_value) {
			if ($_value->getAttrType() == 'multiselect') {
				if (!array_key_exists($_value->getName(), $contactAttrData)) {
					$contactAttrData[$_value->getName()] = array (
						$_value->getValue()
					);
				} else {
	...
	public function deleteItemRelate($itemId) {
		if (Mage :: registry('company_template_module_item')) {
			$relates = Mage :: getResourceModel('edm/company_template_module_item_relate_collection')->addFieldToFilter('item_id', $itemId);

		} else {
			$relates = Mage :: getResourceModel('edm/templates_module_item_relate_collection')->addFieldToFilter('item_id', $itemId);
		}
	...
	public function createItemRelate($type, $name, $itemId) {
		if (Mage :: registry('company_template_module_item')) {
			...
			$model = Mage :: getModel('edm/templates_module_item_relate');
		}

		$model->setData('type', $type)->setData('name', $name)->setData('item_id', $itemId)->save();
		return $model;
	}
