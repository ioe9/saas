 ...
 * 邮件相关操作
 */
class Mage_Adminhtml_Edm_Ajax_EmailController extends Mage_Adminhtml_Controller_Edm
{
	/**
	 * 根据URL分析客户:先抓取
	 * 1.先判断库里是否已有分析好的数据
	 ...
	 * 获取渲染后的邮件
	 */
	public function getSystemDevelopTemplateBak($template) {
		...
		if ($url) {
			...
			if ($url && $client && $client->getId()) {
				...
				$data = $this->getSystemDevelopOutput($client,$emailTo); 
				$output = $data['output'];
				$res['data'] = $output;
	...
	public function getConfigOutput() {
        ...
        $update->load('adminhtml_edm_ajax_email_config');
        $layout->generateXml();
	...
	public function getAnalysisMap($client,$needSphinx=true) {
		...
		$layout = $this->getLayout();
        $update = $layout->getUpdate();
        $update->load('adminhtml_edm_ajax_email_analysis');
        $layout->generateXml();
        $layout->generateBlocks();
        $output = $layout->getOutput();
