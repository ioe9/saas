...
class Mage_Core_Model_Resource_Helper_Mysql4 extends Mage_Core_Model_Resource_Helper_Abstract
{
    ...
    {
        return new Magento_Db_Expr((string)$column);
    }
        ...
        foreach ($columns as $columnEntry) {
            ...
            if ($column instanceof Magento_Db_Expr) {
                ...
                if ($column == Varien_Db_Select::SQL_WILDCARD) {
                    if ($tables[$correlationName]['tableName'] instanceof Magento_Db_Expr) {
                        throw new Zend_Db_Exception("Can't prepare expression when tableName is instance of Magento_Db_Expr");
                    }
...
//        $select->setPart(Varien_Db_Select::COLUMNS, array_values($preparedColumns));
    ...
    public function addGroupConcatColumn($select, $fieldAlias, $fields, $groupConcatDelimiter = ',', $fieldsDelimiter = '', $additionalWhere = '')
    {
        ...
        $select->columns(array($fieldAlias => new Magento_Db_Expr(sprintf('GROUP_CONCAT(%s%s)', $fieldExpr, $separator))));
     ...
     *
     * @param  string|Magento_Db_Expr $startDate
     * @param  string|Magento_Db_Expr $endDate
     * @return Magento_Db_Expr
     */
    public function getDateDiff($startDate, $endDate)
    {
        $dateDiff = '(TO_DAYS(' . $endDate . ') - TO_DAYS(' . $startDate . '))';
        return new Magento_Db_Expr($dateDiff);
    }
    ...
    public function addLikeEscape($value, $options = array())
    {
        ...
        return new Magento_Db_Expr($this->_getReadAdapter()->quote($value));
    }
}
