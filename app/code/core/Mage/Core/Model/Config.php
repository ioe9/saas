...
class Mage_Core_Model_Config extends Mage_Core_Model_Config_Base
{
    ...
    public function __construct($sourceData=null)
    {
        ...
        //各种配置信息
        $this->_options         = new Mage_Core_Model_Config_Options($sourceData); //耗时 8ms
    ...
    public function loadModules()
    {
		...
		//echo "<xmp>";var_dump($this); echo "</xmp>";die();
     ...
     * @return Mage_Core_Model_Config_Element
     */
    public function getNode($path=null,$scope='')
    {
    	if ($scope) {
    		$path = $scope. '/' . $path;
    	}
    ...
    protected function _loadDeclaredModules($mergeConfig = null)
    {
        ...
        foreach ($unsortedConfig->getNode()->children() as $nodeName => $node) {
            ...
            if ($nodeName != 'modules') {
            	...
            	$targetNode = $node->descend(MAGE_TYPE);
            	//echo "<xmp>";var_dump($sortedConfig->getNode('modules'));
            	
            	foreach ($targetNode->children() as $_nodeName => $_node) {
            		$sortedConfig->getNode('modules')->appendChild($_node);
            	}
            	
            }
        }        
        //echo "<xmp>";var_dump($sortedConfig);die();
    ...
    public function loadModulesConfiguration($fileName, $mergeToObject = null, $mergeModel=null)
    {
        ...
        //echo "<xmp>";var_dump($mergeToObject);die();
    ...
    public function getModuleDir($type, $moduleName)
    {
        ...
        $codePool = $codePool ? $codePool : 'core';
        //var_dump($this->getModuleConfig($moduleName));
    ...
    public function getGroupedClassName($groupType, $classId, $groupRootNode=null)
    {
        ...
        if (empty($className)) {
            ...
            if (strstr($group,'_resource')) { 
            	$groupResourde = explode('_',$classId);
        		$className = 'mage_'.$groupResourde[0].'_model_resource';
        	}
            if (empty($className)) {
    ...
    {
        $resource = $this->getResourceModel();
        $resource->deleteConfig(rtrim($path, '/'));
    ...
    protected function _getResourceConnectionModel($moduleName = null)
    {
		...
		//echo '$config:';var_dump($config);die();
    ...
    protected function _getResourceModelFactoryClassName($modelClass)
    {
        ...
        $resourceModel = (string)$module.'_resource';
