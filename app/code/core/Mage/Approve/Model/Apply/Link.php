<?php
class Mage_Approve_Model_Apply_Link extends Mage_Core_Model_Abstract
{
	const LINK_TYPE_TO = 0;
	const LINK_TYPE_CC = 1;
    protected function _construct()
    {
        $this->_init("approve/apply_link");
    }
	 ...
	 * 获取审批关联人
	 */
	public function getApplyLink($applyId,$type = self::LINK_TYPE_TO) {
		$collection = $this->getCollection()
			->addFieldToFilter('link_type',$type)
			->addFieldToFilter('link_apply',$applyId);
	...
	public function getLinkedChooserSelected($applyId,$linkType=self::LINK_TYPE_TO) {
		$collection = Mage::getModel('admin/user')->getUserByCompany();
		$collection->getSelect()
				->join(
	    			array('link'=>'approve_apply_link'),
					'link.link_user=main_table.user_id',
					array('link_type','link_apply')
				);
		$collection->addFieldToFilter('link_apply',$applyId);
