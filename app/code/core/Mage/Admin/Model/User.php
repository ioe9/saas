...
class Mage_Admin_Model_User extends Mage_Core_Model_Abstract
{
    ...
    public function validate()
    {
        ...
        if (!Varien_Validate::is($this->getUsername(), 'NotEmpty')) {
            $errors[] = Mage::helper('adminhtml')->__('用户名不能为空');
        }
        if (!Varien_Validate::is($this->getEmail(), 'EmailAddress')) {
            $errors[] = Mage::helper('adminhtml')->__('请输入正确的邮箱地址');
        }
        ...
        if ($this->userExists()) {
            $errors[] = Mage::helper('adminhtml')->__('对不起，用户名或邮箱不可用，请重试');
        }
    ...
    public function validateCurrentPassword($password)
    {
        ...
        if (!Varien_Validate::is($password, 'NotEmpty')) {
