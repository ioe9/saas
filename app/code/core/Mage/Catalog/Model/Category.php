...
class Mage_Catalog_Model_Category extends Mage_Catalog_Model_Abstract
{
     ...
     */
    const DM_PRODUCT            = 'POST';
    const DM_PAGE               = 'PAGE';
    const DM_MIXED              = 'POST_AND_PAGE';
    const TREE_ROOT_ID          = 1;
    const TREE_DEFAULT_ID          = 2;
    ...
    {
    	//echo get_class($this->getUrlModel());
    ...
    {
        $str = Mage::helper('catalog/post_url')->format($str);
    ...
    public function getAnchorsAbove()
    {
        ...
        if (in_array($this->getId(), $path)) {
            ...
            if (!Mage::registry('_category_is_anchor')) {
                $model = $this->_getAttribute('is_anchor');
                Mage::register('_category_is_anchor', $model);
            }

            if ($isAnchorAttribute = Mage::registry('_category_is_anchor')) {
    ...
    {
        if (!$this->hasPostCount()) {
            $count = $this->_getResource()->getPostCount($this); // load post count
    ...
    /**
     * Retuen siblings categories of current category
     *
     * @return array
     */
    public function getSiblingCategories()
    {
        return $this->getResource()->getSiblingCategories($this);
    }
    ...
    /**
     * Retrieve Available int Post Listing sort by
     *
     * @return null|array
     */
    ...
    /**
     * Retrieve Available Post Listing  Sort By
    ...
    /**
     * Retrieve Post Listing Default Sort By
     *
     * @return string
     */
