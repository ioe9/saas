...
class Mage_Catalog_Model_Resource_Url extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    /**
     * Post attribute properties cache
     *
     * @var array
     */
    protected $_postAttributes           = array();

    /**
     * Limit posts for select
     *
     * @var int
     */
    protected $_postLimit                = 250;
    ...
    {
        return Mage::getSingleton('catalog/post');
    }
    ...
    {
        $adapter = $this->_getWriteAdapter();
        $requestPathField = new Magento_Db_Expr($adapter->quoteIdentifier('request_path'));
        ...
        $select = $adapter->select()
            ->from($this->getMainTable(), new Magento_Db_Expr('MAX(' . $urlIncrementPartExpression . ')'))
            ->where('store_id = :store_id')
     ...
     * @param int|array $postIds
     * @return array
     */
    public function prepareRewrites($storeId, $categoryIds = null, $postIds = null)
    {
        $rewrites   = array();
        $adapter    = $this->_getWriteAdapter();
        $select     = $adapter->select()
        ...
        if ($categoryIds === null) {
            ...
            // Check maybe we request posts and root category id is within categoryIds,
            // it's a separate case because root category posts are stored with NULL categoryId
            if ($postIds) {
        ...
        if ($postIds === null) {
            ...
            $select->where('post_id IN(?)', $postIds);
        }
    ...
    public function getCategories($categoryIds, $storeId)
    {
        ...
        //Mage_Catalog_Block_Post_List_Upsell
    }
    ...
    /**
     * Retrieve post ids by category
     *
     * @param Varien_Object|int $category
     * @return array
     */
    public function getPostIdsByCategory($category)
    {
        ...
        $adapter = $this->_getReadAdapter();
        $select = $adapter->select()
            ->from($this->getTable('catalog/category_post'), array('post_id'))
     ...
     * @return array
     */
    protected function _getPosts($postIds, $storeId, $entityId, &$lastEntityId)
    {
        $posts   = array();
        ...
        $select = $adapter->select()
            ...
            ->from(array('e' => $this->getTable('catalog/post')), array('entity_id'))
            ->join(
                array('w' => $this->getTable('catalog/post_website')),
                'e.entity_id = w.post_id AND w.website_id = :website_id',
                array()
            )
        ...
        $rowSet = $adapter->fetchAll($select, $bind);
        foreach ($rowSet as $row) {
            $post = new Varien_Object($row);
            $post->setIdFieldName('entity_id');
            $post->setCategoryIds(array());
        ...
        if ($posts) {
            $select = $adapter->select()
                ->from(
                    $this->getTable('catalog/category_post'),
                    array('post_id', 'category_id')
                )
                ->where('post_id IN(?)', array_keys($posts));
            $categories = $adapter->fetchAll($select);
            foreach ($categories as $category) {
                $postId = $category['post_id'];
                $categoryIds = $posts[$postId]->getCategoryIds();
                $categoryIds[] = $category['category_id'];
                $posts[$postId]->setCategoryIds($categoryIds);
            }

            foreach (array('name', 'url_key', 'url_path') as $attributeCode) {
                $attributes = $this->_getPostAttribute($attributeCode, array_keys($posts), $storeId);
                foreach ($attributes as $postId => $attributeValue) {
                    $posts[$postId]->setData($attributeCode, $attributeValue);
                }
            }
        }
     ...
     * @return Varien_Object
     */
    public function getPost($postId, $storeId)
    {
        $entityId = 0;
        $posts = $this->_getPosts($postId, $storeId, 0, $entityId);
    ...
    /**
     * Retrieve Post data obects for store
     *
     * @param int $storeId
     * @param int $lastEntityId
     * @return array
     */
    public function getPostsByStore($storeId, &$lastEntityId)
    {
        return $this->_getPosts(null, $storeId, $lastEntityId, $lastEntityId);
    }

    /**
     * Retrieve Post data objects in category
     *
     * @param Varien_Object $category
     * @param int $lastEntityId
     * @return array
     */
    public function getPostsByCategory(Varien_Object $category, &$lastEntityId)
    {
        $postIds = $this->getPostIdsByCategory($category);
        if (!$postIds) {
            return array();
        }
        return $this->_getPosts($postIds, $category->getStoreId(), $lastEntityId, $lastEntityId);
    }

    /**
     * Find and remove unused posts rewrites - a case when posts were moved away from the category
     * (either to other category or deleted), so rewrite "category_id-post_id" is invalid
     *
     * @param int $storeId
     * @return Mage_Catalog_Model_Resource_Url
     */
    public function clearCategoryPost($storeId)
    {
        $adapter = $this->_getWriteAdapter();
        $select = $adapter->select()
            ->from(array('tur' => $this->getMainTable()), $this->getIdFieldName())
            ->joinLeft(
                array('tcp' => $this->getTable('catalog/category_post')),
                'tur.category_id = tcp.category_id AND tur.post_id = tcp.post_id',
                array()
            )
            ->where('tur.store_id = :store_id')
            ->where('tur.category_id IS NOT NULL')
            ->where('tur.post_id IS NOT NULL')
    ...
    /**
     * Remove unused rewrites for post - called after we created all needed rewrites for post and know the categories
     * where the post is contained ($excludeCategoryIds), so we can remove all invalid post rewrites that have other category ids
     *
     * Notice: this routine is not identical to clearCategoryPost(), because after checking all categories this one removes rewrites
     * for post still contained within categories.
     *
     * @param int $postId Post entity Id
     * @param int $storeId Store Id for rewrites
     * @param array $excludeCategoryIds Array of category Ids that should be skipped
     * @return Mage_Catalog_Model_Resource_Url
     */
    public function clearPostRewrites($postId, $storeId, $excludeCategoryIds = array())
    {
        $where = array(
            'post_id = ?' => $postId,
            'store_id = ?' => $storeId
        ...
        if (!empty($excludeCategoryIds)) {
            ...
            // If there's at least one category to skip, also skip root category, because post belongs to website
    ...
    /**
     * Finds and deletes all old category and category/post rewrites for store
     * left from the times when categories/posts belonged to store
     *
     * @param int $storeId
    ...
    public function clearStoreCategoriesInvalidRewrites($storeId)
    {
        ...
        // Remove all store catalog rewrites that are for some category or cartegory/post not within store categories
        $where   = array(
            'store_id = ?' => $storeId,
    ...
    /**
     * Finds and deletes post rewrites (that are not assigned to any category) for store
     * left from the times when post was assigned to this store's website and now is not assigned
     *
     * Notice: this routine is different from clearPostRewrites() and clearCategoryPost() because
     * it handles direct rewrites to post without defined category (category_id IS NULL) whilst that routines
     * handle only post rewrites within categories
     *
     * @param int $storeId
     * @param int|array|null $postId
     * @return Mage_Catalog_Model_Resource_Eav_Mysql4_Url
     */
    public function clearStorePostsInvalidRewrites($storeId, $postId = null)
    {
        $store   = $this->getStores($storeId);
        $adapter = $this->_getReadAdapter();
        $bind    = array(
        ...
        $select = $adapter->select()
            ...
            ->joinLeft(
                array('website' => $this->getTable('catalog/post_website')),
                'rewrite.post_id = website.post_id AND website.website_id = :website_id',
                array()
            )->where('rewrite.store_id = :store_id')
            ->where('rewrite.category_id IS NULL');
        if ($postId) {
            $select->where('rewrite.post_id IN (?)', $postId);
        } else {
            $select->where('rewrite.post_id IS NOT NULL');
        }
     ...
     * b) post rewrites left from posts that once belonged to this site, but then deleted or just removed from website
     *
     * @param int $storeId
     * @return Mage_Catalog_Model_Resource_Eav_Mysql4_Url
     */
    public function clearStoreInvalidRewrites($storeId)
    {
        ...
        $this->clearStorePostsInvalidRewrites($storeId);
        return $this;
    }

    /**
     * Delete rewrites for associated to category posts
     *
     * @param int $categoryId
     * @param array $postIds
     * @return Mage_Catalog_Model_Resource_Url
     */
    public function deleteCategoryPostRewrites($categoryId, $postIds)
    {
        $this->deleteCategoryPostStoreRewrites($categoryId, $postIds);
        return $this;
    }

    /**
     * Delete URL rewrites for category posts of specific store
     *
     * @param int $categoryId
     * @param array|int|null $postIds
     * @param null|int $storeId
     * @return Mage_Catalog_Model_Resource_Url
     */
    public function deleteCategoryPostStoreRewrites($categoryId, $postIds = null, $storeId = null)
    {
        // Notice that we don't include category_id = NULL in case of root category,
        // because post removed from all categories but assigned to store's website is still
        // assumed to be in root cat. Unassigned posts must be removed by other routine.
        $condition = array('category_id = ?' => $categoryId);
        if (empty($postIds)) {
            $condition[] = 'post_id IS NOT NULL';
        } else {
     ...
     * Input array format:
     * post_id as key and store_id as value
     * Output array format (post_id as key)
     ...
     * @return array
     */
    public function getRewriteByPostStore(array $posts)
    {
        $result = array();
        ...
        $select = $adapter->select()
            ->from(
                array('i' => $this->getTable('catalog/category_post_index')),
                array('post_id', 'store_id', 'visibility')
            )
            ->joinLeft(
                array('r' => $this->getMainTable()),
                'i.post_id = r.post_id AND i.store_id=r.store_id AND r.category_id IS NULL',
                array('request_path')
            );

        $bind = array();
        foreach ($posts as $postId => $storeId) {
            $catId = Mage::app()->getStore($storeId)->getRootCategoryId();
            $postBind = 'post_id' . $postId;
