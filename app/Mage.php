...
final class Mage
{
    ...
    /**
     * Mage edition constants
     */
    const EDITION_COMMUNITY    = 'Community';

    /**
     * Current Mage edition.
     *
     * @var string
     * @static
     */
    ...
    /**
     * Gets the current Mage version string
     * @link http://www.Magecommerce.com/blog/new-community-edition-release-process/
     *
     * @return string
     */
    ...
    /**
     * Gets the detailed Mage version information
     * @link http://www.Magecommerce.com/blog/new-community-edition-release-process/
     *
     * @return array
     */
    ...
    {
        return self::app()->getStoreConfig($path);
    }
    ...
    {
        $flag = strtolower(self::getStoreConfig($path));
     ...
     */
    public static function init($options = array(), $modules = array())
    {
        try {
            self::setRoot();
            self::$_app     = new Mage_Core_Model_App();
            ...
            if (!empty($modules)) {
                self::$_app->initSpecified($options, $modules);
            } else {
                self::$_app->init($options);
            }
    ...
    public static function run($options = array())
    {
        try {
            ...
            self::_setConfigModel($options); //耗时
    ...
    public static function log($message, $level = null, $file = '', $forceLog = false)
    {
        ...
        $level  = is_null($level) ? Varien_Log::DEBUG : $level;
        ...
        try {
            if (!isset($loggers[$file])) {
                ...
                $formatter = new Varien_Log_Formatter_Simple($format);
                $writerModel = (string)self::getConfig()->getNode('global/log/core/writer_model');
                if (!self::$_app || !$writerModel) {
                    $writer = new Varien_Log_Writer_Stream($logFile);
                }
                else {
                    $writer = new $writerModel($logFile);
                }
                $writer->setFormatter($formatter);
                $loggers[$file] = new Varien_Log($writer);
            }
